...
function registerResponsesFunctionCallState(
  ...
  if (!callState) {
    const outputIndex = metadata.outputIndex !== null ? metadata.outputIndex : context.nextToolIndex
    callState = {
      outputIndex,
      callId: '',
      itemId: '',
...
async function handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  ...
  // Copilot passthrough is not supported in PetClaw (no GitHub Copilot integration).
  if (
    method === 'POST' &&
    ...
    writeJSON(
      res,
      501,
      createAnthropicErrorBody('GitHub Copilot passthrough is not supported', 'not_implemented')
    )
    return
  }

  // OpenClaw sends requests to /v1/chat/completions (OpenAI format) when using
  // the petclaw provider. Transparently proxy these requests to the upstream.
  if (
    method === 'POST' &&
  ...
  // Inject session_id and user_message for petclaw-server logging only.
  // Strict providers (e.g. Gemini) reject unknown payload fields.
  if (upstreamConfig.provider === 'petclaw-server') {
