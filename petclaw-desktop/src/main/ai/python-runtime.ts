...
// 嵌入式 Python 的 ._pth 文件默认注释掉了 import site，
// 需要启用 site-packages 支持以便安装第三方包
...
// Windows PATH 拼接：新条目优先，去重（大小写不敏感）
...
export function appendPythonRuntimeToEnv(
  ...
  if (pathEntries.length > 0) {
    ...
    env.PETCLAW_PYTHON_ROOT = pathEntries[0]
  }
...
// 确保 Python 运行时已就绪：
// 1. 若 userData 中已有健康的运行时，直接使用
// 2. 否则从应用包内复制并修复 _pth 配置
export async function ensurePythonRuntimeReady(): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    if (userHealth.ok) {
      ...
      console.warn('[python-runtime] User runtime already healthy')
      return { success: true }
    }
    ...
    console.warn(`[python-runtime] Sync runtime to userData: ${userRoot}`)
    if (fs.existsSync(userRoot)) {
    ...
    console.warn('[python-runtime] Runtime sync complete')
    return { success: true }
  } catch (error) {
...
// 确保 pip 可用：先保证 runtime 健康，再尝试 ensurepip 自举
export async function ensurePythonPipReady(): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    if (bootstrapResult.ok) {
      ...
      if (finalHealth.ok) {
        console.warn('[python-runtime] ensurepip successfully restored pip in user runtime')
        return { success: true }
      }
    }
