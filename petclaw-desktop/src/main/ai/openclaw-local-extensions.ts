...
const findBundledExtensionsDir = (): string | null => {
  const candidates = app.isPackaged
    ? [path.join(process.resourcesPath, 'petmind', THIRD_PARTY_EXTENSIONS_DIR)]
    : [
        path.join(
          app.getAppPath(),
          'vendor',
...
export const listBundledOpenClawExtensionIds = (): string[] => {
  ...
  try {
    return fs
      ...
      .filter((entry) => entry.isDirectory())
      .filter((entry) =>
        fs.existsSync(path.join(extensionsDir, entry.name, 'openclaw.plugin.json'))
      )
      .map((entry) => entry.name)
  } catch {
    return []
 ...
 * 2. `extensions/{id}` — prior versions of PetClaw installed plugins here.
