...
export function registerMemoryIpcHandlers(deps: MemoryIpcDeps): void {
  ...
  ipcMain.handle('memory:read', async (_event, workspace: string) =>
    memoryManager.readMemory(workspace)
  )

  // 以 Markdown 列表行形式追加一条记忆条目
  ipcMain.handle('memory:append', async (_event, workspace: string, entry: string) => {
    memoryManager.appendMemory(workspace, entry)
  })

  // 删除包含指定文本的记忆行
  ipcMain.handle('memory:remove', async (_event, workspace: string, text: string) => {
    memoryManager.removeMemory(workspace, text)
  })

  // 按关键词搜索记忆（大小写不敏感），返回匹配行数组
  ipcMain.handle('memory:search', async (_event, workspace: string, keyword: string) =>
    memoryManager.searchMemory(workspace, keyword)
  )

  // 列出所有以 '- ' 开头的条目，附带行号（1-indexed）
  ipcMain.handle('memory:list-entries', async (_event, workspace: string) =>
