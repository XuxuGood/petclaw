...
export function CronEditDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay p-6">
      <div className="bg-bg-root rounded-[12px] shadow-lg w-[min(540px,calc(100vw-48px))] max-h-[calc(100vh-48px)] flex flex-col overflow-hidden">
        {/* 标题 */}
        <div className="flex items-center justify-between px-6 py-5">
          ...
          <button
            onClick={onClose}
            className="min-h-[var(--size-control-min)] min-w-[var(--size-control-min)] rounded-[8px] hover:bg-bg-hover transition-colors shrink-0 ui-focus"
          >
        ...
        {/* 内容 */}
        <div className="ui-contained-scroll flex-1 overflow-y-auto px-6 pb-4">
          {/* 任务名称 */}
          <div className="mb-5">
            ...
            <input
              ...
              className="w-full px-4 py-3 text-[14px] rounded-[8px] bg-bg-root border border-border text-text-primary placeholder:text-text-tertiary focus:outline-none focus:ring-1 focus:ring-accent/40"
            />
          </div>
          ...
          <div className="mb-5">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <select
                ...
                className="px-4 py-3 text-[14px] rounded-[8px] bg-bg-root border border-border text-text-primary focus:outline-none focus:ring-1 focus:ring-accent/40 min-w-[120px]"
              >
              ...
              {frequency !== 'custom' ? (
                <div className="relative shrink-0">
                  <input
                    ...
                    className="px-4 py-3 pr-10 text-[14px] rounded-[8px] bg-bg-root border border-border text-text-primary focus:outline-none focus:ring-1 focus:ring-accent/40"
                  />
                  <Clock
                    size={16}
                ...
                <input
                  ...
                  placeholder="0 9 * * 1-5"
                  className="flex-1 px-4 py-3 text-[14px] font-mono rounded-[8px] bg-bg-root border border-border text-text-primary placeholder:text-text-tertiary focus:outline-none focus:ring-1 focus:ring-accent/40"
                />
              )}
            </div>
                ...
                {weekdayLabels.map((label, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-9 h-9 rounded-[8px] text-[13px] font-medium transition-all duration-[120ms] active:scale-[0.96] ui-focus ${
                        isActive
          ...
          <div className="mb-4">
            <div className="border border-border rounded-[8px] overflow-hidden">
              ...
              />
              {/* 工具栏 */}
              <div className="flex flex-wrap items-center gap-2 px-3 py-2 border-t border-border bg-bg-root">
                <button
                  onClick={handleSelectCwd}
                  className="flex min-h-[var(--size-control-min)] min-w-0 items-center gap-1.5 px-2.5 py-1.5 text-[12px] text-text-secondary hover:text-text-primary hover:bg-bg-hover rounded-[8px] transition-colors ui-focus"
                >
                  <FolderOpen size={14} />
                ...
                <button className="min-h-[var(--size-control-min)] min-w-[var(--size-control-min)] text-text-tertiary hover:text-text-secondary rounded-[8px] hover:bg-bg-hover transition-colors ui-focus">
                  <Calendar size={16} />
                </button>
                <button className="min-h-[var(--size-control-min)] min-w-[var(--size-control-min)] text-text-tertiary hover:text-text-secondary rounded-[8px] hover:bg-bg-hover transition-colors ui-focus">
                  <Paperclip size={16} />
                ...
                <button className="flex min-h-[var(--size-control-min)] items-center gap-1 px-2 py-1 text-[12px] text-text-tertiary hover:text-text-secondary rounded-[8px] hover:bg-bg-hover transition-colors ui-focus">
        ...
        {/* 底部按钮 */}
        <div className="flex flex-wrap justify-end gap-3 px-6 py-4 border-t border-border">
          <button
            onClick={onClose}
            className="min-h-[var(--size-control-min)] px-5 py-2.5 text-[14px] text-text-secondary hover:text-text-primary transition-colors active:scale-[0.96] duration-[120ms] ui-focus"
          >
            {t('common.cancel')}
          </button>
          <button
            ...
            className="min-h-[var(--size-control-min)] px-5 py-2.5 text-[14px] rounded-[8px] bg-text-primary text-white hover:opacity-90 transition-all active:scale-[0.96] duration-[120ms] disabled:opacity-40 ui-focus"
          >
