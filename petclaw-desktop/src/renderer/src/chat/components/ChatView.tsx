    ...
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto">
        {isEmpty ? (
          ...
          <div className="max-w-[680px] mx-auto px-6 py-2 space-y-5">
            {messages.map((msg) => (
              <div
                key={msg.id}
      ...
      <div className="shrink-0 px-6 pb-4 pt-3">
        <div className="max-w-[680px] mx-auto">
          <div className="flex items-end gap-2.5 bg-bg-input rounded-[14px] px-4 py-2.5 border border-border-input">
            <input
              type="text"
              value={input}
              ...
              placeholder="输入消息..."
              className="flex-1 text-[13.5px] text-text-primary bg-transparent outline-none placeholder:text-text-tertiary py-1"
              disabled={isLoading}
            />
            <button
              onClick={() => handleSend()}
              disabled={isLoading || !input.trim()}
              className="w-7 h-7 rounded-full bg-accent text-white flex items-center justify-center shrink-0 hover:bg-accent-hover disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-150 ease active:scale-[0.94]"
              aria-label="发送消息"
            >
...
function EmptyState({ onSend }: { onSend: (text: string) => void }): JSX.Element {
  return (
      ...
      <div className="flex flex-wrap gap-2 justify-center max-w-md">
        {SUGGESTED_PROMPTS.map((prompt) => (
          <button
            ...
            className="px-4 py-2 rounded-[10px] bg-bg-card text-[13px] text-text-secondary shadow-[var(--shadow-card)] border border-border hover:border-text-tertiary hover:shadow-[var(--shadow-dropdown)] transition-all duration-150 ease active:scale-[0.97]"
          >
            {prompt}
          </button>
        ))}
