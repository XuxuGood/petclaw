...
**日期**: 2026-04-22（2026-04-26 更新：Agent-centric → Directory-centric 架构重构）
**状态**: 设计完成，待实现
**范围**: 运行时管理、目录驱动 Agent、Cowork 模式、模型配置、持久记忆、MCP 服务器、IM 机器人、定时任务、聊天输入框、Openclaw 版本管理、开发到上线流程
...
export class ConfigSync {
  ...
  sync(reason: string): SyncResult {
    const config = {
      ...
      agents: this.directoryManager.toOpenclawConfig(),  // directories → agents.list
      skills: {
...
workspace = 用户配置的默认工作目录（`app_config['defaultDirectory']`，默认 `{userData}/openclaw/state/workspace/`）
...
每个目录的 workspace 直接指向该目录路径本身（通过 agents.list[i].workspace = directory.path）。不再创建 `workspace-{agentId}` 子目录。

```typescript
private syncPerDirectoryWorkspaces(): void {
  for (const dir of this.directoryManager.list()) {
    // 目录的 workspace = 目录路径本身（agents.list[i].workspace = dir.path）
    // AGENTS.md 写入目录下（如果用户允许）或 {STATE_DIR}/workspace-{agentId}/
    // 这样 Agent 可以访问目录下的项目文件
  }
}
```
...
export class DirectoryManager extends EventEmitter {
  ...
  // 生成结构：
  // {
  //   defaults: { timeoutSeconds, model, workspace: defaultDirectory },
  //   list: [
...
```jsonc
// openclaw.json agents 部分（由 DirectoryManager.toOpenclawConfig() 生成）
{
  "agents": {
    "defaults": {
      "timeoutSeconds": 3600,
      "model": { "primary": "claude-sonnet-4-20250514" },
      "workspace": "/Users/xxx/default-project"  // app_config['defaultDirectory']
    },
    "list": [
      { "id": "main", "default": true },
      {
...
- 激活状态存前端 Zustand `activeSkillIds`，不持久化到 DB
- 发送消息时附带 `activeSkillIds[]`
...
app.whenReady()
  ...
  → runBootCheck(chatWindow)
      ...
      ├── ModelRegistry.load()
      ├── McpManager.load()                       // ★
      ├── ConfigSync.sync('boot')                 // 生成 openclaw.json（从 directories 表聚合 agents.list）
...
```
Renderer ChatInputBox → IPC chat:send { text, cwd, files, activeSkillIds }
...
- `mcp/mcp-manager.ts` — McpManager（全局启用，无会话级隔离）
...
- McpManager 全局 CRUD → DB 持久化 → ConfigSync 同步（无会话级隔离）
