...
- 随反馈提交的诊断日志必须具备人类可读英文 `message`，不能只有机器事件 key。
...
Feedback 依赖 Logging 的人类可读性约定：

- `event` 是稳定机器 key，例如 `titleGeneration.failed`。
- `message` 是固定英文人类可读句子，例如 `Failed to generate session title`。
- `fields` 承载变量，例如 `sessionId`、`modelId`、`elapsedMs`。
- error/warn 级日志必须提供 `message`；变量不得拼接进 `message`。
- 反馈弹窗的诊断摘要可展示日志源、级别、模块、message、event 和大小，不展示日志原文。
