...
Agent 层只输出 agent 列表、身份和技能绑定，不负责模型选择。模型统一由 ChatInputBox 的 `selectedModel` 经 `sessions.patch` 生效；全局默认模型只由 `ConfigSync` 写入 `agents.defaults.model.primary`，作为非 UI 入口的运行时默认值。

```typescript
toOpenclawConfig(): OpenclawAgentsConfig {
  return {
    list: this.list()
      .filter(agent => agent.enabled && !agent.isDefault)
      .map(agent => ({
        id: agent.id,
  ...
  sync(reason: string): ConfigSyncResult {
    const config = {
      ...
      agents: {
        ...
        ...this.agentManager.toOpenclawConfig(),
      },
...
模型选择不作为 `chat.send` 参数传给 OpenClaw。ChatInputBox 必须始终有一个当前选中模型：优先取当前目录 Agent 的模型覆盖，其次取 `app_config.model` 的全局默认模型。前端传给主进程的是 PetClaw 内部结构 `{ providerId, modelId }`，主进程通过 `ModelRegistry.toOpenClawModelRef()` 转成 OpenClaw model ref，再通过 `sessions.patch` 写入 OpenClaw session。
...
2. `directory.modelOverride`：当前目录 Agent 的模型覆盖，用于初始化 ChatInputBox。
...
用户点击「+ 新任务」
  ...
  └→ 3. 用户在输入框输入消息并发送
       │
       ├→ chat:send IPC (message, cwd, currentAgentId, skillIds, selectedModel)
...
用户输入消息 + 选择 cwd/skills/model
  → chat:send IPC (message, cwd, agentId, skillIds, selectedModel)
  → SessionManager.createAndStart(title, cwd, prompt, { agentId, skillIds, selectedModel })
