...
参考 LobsterAI `CoworkPromptInput.tsx`，工具栏统一放在输入框**下方**：
...
│                                              │
├──────────────────────────────────────────────┤
│ [📁 ~/projects] [🔧 Skills +2] [🔌 MCP] [📎] [↑] │
└──────────────────────────────────────────────┘
```

工具栏从左到右：工作目录选择、Skills 多选、MCP 管理、附件添加、发送按钮。
...
**会话列表联动**：切换 Agent 后，侧栏下方的会话列表**只显示当前 Agent 的会话记录**。每个 Agent 的会话相互隔离，互不干扰。
...
切换 Agent 后，主展示区自动进入该 Agent 的聊天界面（新会话或最近会话），无需额外点击"新建会话"。
