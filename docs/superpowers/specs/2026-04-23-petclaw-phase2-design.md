# PetClaw v3 Phase 2 — 核心功能设计

**日期**: 2026-04-23
**状态**: 设计完成，待实现
**范围**: DirectoryManager、ModelRegistry、SkillManager、McpManager、IM、多来源 Cowork session、ConfigSync、CoworkSessionManager、IPC、前端任务中心侧栏、ChatInputBox
**参考**: LobsterAI 源码 `/Users/xiaoxuxuy/Desktop/工作/AI/开源项目/LobsterAI`

**2026-05-02 有效性说明**：本文件早期章节仍记录 Phase 2 的 Agent-centric 设计历史。Phase 5 已改为 Directory-driven Agent，用户不直接管理 Agent；当前仍有效的前端方向以第 12 章为准：左侧栏是任务中心，工作目录只作为任务上下文，频道按 IM 实例和对话来源分组展示。

---

## 1. 设计目标

在 Phase 1 基础架构（EngineManager / Gateway / CoworkController）之上，构建**核心功能层**：

1. **Directory-driven Agent** — 用户管理工作目录；`agentId` 由目录派生，main agent 作为无目录任务的默认运行时
2. **多 Provider 多 Model** — 11 个预设提供商 + 自定义，API Key 安全存储，测试连接
3. **Skills 管理** — 扫描 + 启用/禁用，通过 openclaw.json 原生加载
4. **MCP 服务器** — stdio/sse/streamable-http 三种传输，DB 持久化
5. **持久记忆** — 纯文件驱动（MEMORY.md + daily notes）
6. **ConfigSync 升级** — 从 deps 注入重构为直接依赖 Manager，新增 workspace 文件同步
7. **前端完整改造** — Settings 分 Tab 页面 + ChatInputBox 增强

---

## 2. 架构概览

```
Phase 1 基础层（已完成）:
  OpenclawEngineManager → OpenclawGateway → CoworkController → CoworkStore

Phase 2/Phase 5 有效功能层:
  DirectoryManager ─┐
  ModelRegistry ────┤
  SkillManager ─────┼→ ConfigSync（聚合）→ openclaw.json / AGENTS.md / exec-approvals.json
  McpManager ───────┤
  ImGatewayManager ─┤
  CronJobService ───┘
  MemorySearchConfigStore（独立配置）

Phase 2 升级:
  ConfigSync    — deps 注入 → 直接依赖 Manager
  CoworkSessionManager — 支持目录派生 agentId + sessionKey 格式
  IPC           — cowork/directory/models/skills/mcp/im channels
  前端           — Settings 分 Tab + ChatInputBox 增强
```

**关键设计决策**：

| 决策 | 选择 | 原因 |
|------|------|------|
| ConfigSync 依赖方式 | 直接依赖 Manager | ConfigSync 是聚合层，deps 注入增加不必要间接性 |
| DB 组织 | Manager 自包含 CRUD | 每个 Manager 自治，不需要集中 CRUD 层 |
| API Key 存储 | `model_provider_secrets` + 环境变量注入 | Key 不写入 openclaw.json，通过 `${VAR}` 占位符；后续可替换为 Keychain |
| Skills 加载 | openclaw.json `skills.load.extraDirs` | Openclaw runtime 原生扫描，不需要手动注入 |

---

## 3. DB Schema 扩展

在 `data/db.ts` 的 `initDatabase()` 中新增业务实体表。`app_config` 继续承载应用级偏好，模型 Provider 和 API Key 不再塞进单个 KV JSON，避免后续迁移、局部更新和 OpenClaw 同步逻辑被大 JSON 绑定。

### 3.1 agents 表

```sql
CREATE TABLE IF NOT EXISTS agents (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  description TEXT NOT NULL DEFAULT '',
  system_prompt TEXT NOT NULL DEFAULT '',
  identity TEXT NOT NULL DEFAULT '',
  model TEXT NOT NULL DEFAULT '',
  icon TEXT NOT NULL DEFAULT '',
  skill_ids TEXT NOT NULL DEFAULT '[]',
  enabled INTEGER NOT NULL DEFAULT 1,
  is_default INTEGER NOT NULL DEFAULT 0,
  source TEXT NOT NULL DEFAULT 'custom',
  preset_id TEXT NOT NULL DEFAULT '',
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL
);
```

### 3.2 mcp_servers 表

```sql
CREATE TABLE IF NOT EXISTS mcp_servers (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL UNIQUE,
  description TEXT NOT NULL DEFAULT '',
  enabled INTEGER NOT NULL DEFAULT 1,
  transport_type TEXT NOT NULL DEFAULT 'stdio',
  config_json TEXT NOT NULL DEFAULT '{}',
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL
);
```

### 3.3 model_providers 表

保存用户可变的 Provider 配置。内置 Provider 的默认定义来自代码里的 `ProviderRegistry`，数据库只保存用户覆盖值和自定义 Provider。

```sql
CREATE TABLE IF NOT EXISTS model_providers (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  base_url TEXT NOT NULL,
  api_format TEXT NOT NULL,
  enabled INTEGER NOT NULL DEFAULT 0,
  is_custom INTEGER NOT NULL DEFAULT 0,
  models_json TEXT NOT NULL DEFAULT '[]',
  created_at INTEGER NOT NULL,
  updated_at INTEGER NOT NULL
);
```

`models_json` 先保留 JSON：模型列表通常跟 Provider 一起编辑、一起同步，当前没有模型级复杂查询需求。后续只有出现模型级启用/禁用、排序、价格、能力标签、使用统计等需求时，再拆 `model_definitions` 表。

### 3.4 model_provider_secrets 表

单独保存 API Key。这样 API Key 不混入 Provider 配置 JSON，也为后续切换 macOS Keychain 或其他本机安全存储保留清晰边界。

```sql
CREATE TABLE IF NOT EXISTS model_provider_secrets (
  provider_id TEXT PRIMARY KEY,
  api_key TEXT NOT NULL DEFAULT '',
  updated_at INTEGER NOT NULL
);
```

### 3.5 app_config 中的模型偏好

`app_config` 只保存全局默认选择，不保存完整 Provider 列表：

```json
{
  "model": {
    "defaultProviderId": "petclaw",
    "defaultModelId": "petclaw-fast"
  }
}
```

PetClaw 自有套餐模型属于内置/服务端模型 catalog，不直接塞入 `app_config`。`app_config` 只保存用户当前选择和套餐状态快照，例如 plan id、权益版本、最后同步时间。

---

## 4. DirectoryManager

### 4.1 文件位置

`src/main/ai/directory-manager.ts`

### 4.2 接口设计

```typescript
export interface DirectoryRecord {
  path: string
  name: string
  agentId: string
  skillIds: string[]
  modelOverride: string | null
  createdAt: number
  updatedAt: number
}

export class DirectoryManager extends EventEmitter {
  constructor(private db: Database.Database) {}

  ensureRegistered(path: string): DirectoryRecord
  list(): DirectoryRecord[]
  get(agentId: string): DirectoryRecord | null
  getByPath(path: string): DirectoryRecord | null
  updateName(agentId: string, name: string): void
  updateModel(agentId: string, model: string | null): void
  updateSkills(agentId: string, skillIds: string[]): void
  toOpenclawAgents(): OpenclawAgentConfig[]

  on('change', () => void): this
}
```

### 4.3 目录与 agentId

用户不直接创建、切换或删除 Agent。PetClaw 用目录配置驱动 OpenClaw runtime：

- `main`：无显式目录且无默认目录时使用，workspace 指向 `{userData}/openclaw/workspace`。
- `ws-{hash}`：由目录路径稳定派生，写入 `directories.agent_id` 和 `cowork_sessions.agent_id`。
- `DirectoryManager.ensureRegistered(cwd)` 是幂等操作；Cowork session 创建时由主进程调用。
- 目录级 `skillIds` 和 `modelOverride` 是默认值；会话级选择可覆盖模型和 skills。

### 4.4 toOpenclawAgents()

Directory 层只输出目录 agents.list，不负责全局默认模型。默认模型由 `ConfigSync` 写入 `agents.defaults.model.primary`，作为非 UI 入口的运行时默认值；普通 UI 会话启动后仍通过 `sessions.patch` 写入明确模型。

```typescript
toOpenclawAgents(): OpenclawAgentConfig[] {
  return this.list().map((directory) => ({
    id: directory.agentId,
    workspace: directory.path,
    identity: { name: directory.name },
    ...(directory.skillIds.length > 0 ? { skills: directory.skillIds } : {}),
  }))
}
```

---

## 5. 模型配置架构

### 5.1 文件位置

```text
src/shared/models/provider-registry.ts       # 内置 Provider / OpenClaw 映射 / 默认模型
src/main/models/model-config-store.ts        # SQLite 持久化
src/main/models/model-registry.ts            # 业务服务与 OpenClaw 配置生成
```

### 5.2 职责分层

| 模块 | 职责 | 持久化 |
|------|------|--------|
| `ProviderRegistry` | 内置 Provider 事实源：展示名、默认 baseUrl、默认 apiFormat、OpenClaw provider id、默认模型、是否无 Key | 代码 |
| `ModelConfigStore` | 读写 `model_providers`、`model_provider_secrets`、`app_config.model` | SQLite |
| `ModelRegistry` | 合并内置默认值和用户配置，校验默认模型，生成 OpenClaw config，收集环境变量 | 无直接存储 |

这个拆分避免旧实现中静态预设 Provider、KV 大 JSON、OpenClaw provider id 三套概念混在一起。Provider 的“默认事实”必须来自 Registry；用户设置只保存覆盖值。

### 5.3 类型设计

```typescript
export interface ProviderDefinition {
  id: string                // PetClaw 内部 ID：'petclaw', 'openai', 'zhipu'
  openClawProviderId: string // OpenClaw ID：如 zhipu -> zai, gemini -> google
  name: string
  logo: string
  defaultBaseUrl: string
  apiFormat: 'openai-completions' | 'anthropic'
  auth: 'api-key' | 'none'
  isPreset: boolean
  defaultModels: ModelDefinition[]
}

export interface ModelProviderConfig {
  id: string
  name: string
  baseUrl: string
  apiFormat: 'openai-completions' | 'anthropic'
  enabled: boolean
  isCustom: boolean
  models: ModelDefinition[]
  hasApiKey: boolean
}

export interface ModelDefinition {
  id: string                // 'gpt-4o'
  name: string
  reasoning: boolean
  supportsImage: boolean
  contextWindow: number
  maxTokens: number
}

export interface ModelPreference {
  defaultProviderId: string
  defaultModelId: string
}
```

`ModelProviderConfig` 返回给渲染进程时只暴露 `hasApiKey`，不返回明文 API Key。设置 API Key 走单独 IPC，主进程写入 `model_provider_secrets`。

### 5.4 ModelRegistry 接口

```typescript
export class ModelRegistry extends EventEmitter {
  constructor(
    private store: ModelConfigStore,
    private providerRegistry: ProviderRegistry,
  ) {}

  // Provider CRUD
  addProvider(provider: CreateCustomProviderInput): void
  updateProvider(id: string, patch: UpdateProviderInput): void
  removeProvider(id: string): void    // 预设不可删除
  toggleProvider(id: string, enabled: boolean): void
  listProviders(): ModelProviderConfig[]
  getProvider(id: string): ModelProviderConfig | undefined

  // 模型 CRUD（每个 Provider 下的模型列表）
  addModel(providerId: string, model: ModelDefinition): void
  removeModel(providerId: string, modelId: string): void
  updateModel(providerId: string, modelId: string, patch: Partial<ModelDefinition>): void

  // 默认模型偏好，持久化到 app_config.model
  setDefaultModel(providerId: string, modelId: string): void
  getDefaultModel(): { provider: ModelProviderConfig; model: ModelDefinition }
  getDefaultOpenClawModelRef(): string  // 'openai/gpt-4o' 或 'zai/glm-4-plus'

  // API Key，持久化到 model_provider_secrets
  setApiKey(providerId: string, apiKey: string): void
  clearApiKey(providerId: string): void

  // 测试连接（发送简单请求验证 API Key）
  async testConnection(providerId: string): Promise<{ ok: boolean; error?: string; latencyMs?: number }>

  // 序列化到 openclaw.json
  toOpenclawConfig(): OpenclawModelsConfig

  // API Key 收集（注入 Gateway 环境变量）
  collectSecretEnvVars(): Record<string, string>

  on('change', () => void): this
}
```

### 5.5 预设 Provider

内置 Provider 必须集中定义在 `ProviderRegistry`，新增 Provider 只改这一处。Registry 必须同时定义 PetClaw 内部 id 和 OpenClaw provider id，避免把 UI/业务 id 误写进 `openclaw.json`。

| PetClaw ID | OpenClaw ID | 名称 | API 格式 | Base URL |
|----|----|------|----------|----------|
| `petclaw` | `petclaw` | PetClaw | openai-completions | `https://petclaw.ai/api/v1` |
| `openai` | `openai` | OpenAI | openai-completions | `https://api.openai.com/v1` |
| `anthropic` | `anthropic` | Anthropic | anthropic | `https://api.anthropic.com` |
| `deepseek` | `deepseek` | DeepSeek | openai-completions | `https://api.deepseek.com/v1` |
| `zhipu` | `zai` | 智谱 Zhipu | openai-completions | `https://open.bigmodel.cn/api/paas/v4` |
| `minimax` | `minimax` | MiniMax | openai-completions | `https://api.minimax.chat/v1` |
| `volcengine` | `volcengine` | 火山引擎 Volcengine | openai-completions | `https://ark.cn-beijing.volces.com/api/v3` |
| `youdao` | `youdaozhiyun` | 有道 Youdao | openai-completions | `https://api.youdao.com/v1` |
| `qianfan` | `qianfan` | 百度千帆 Qianfan | openai-completions | `https://aip.baidubce.com/rpc/2.0/ai_custom/v1` |
| `stepfun` | `stepfun` | 阶跃星辰 StepFun | openai-completions | `https://api.stepfun.com/v1` |
| `xiaomi` | `xiaomi` | 小米 Xiaomi | openai-completions | `https://api.xiaomi.com/v1` |
| `ollama` | `ollama` | Ollama | openai-completions | `http://localhost:11434/v1` |
| `gemini` | `google` | Google Gemini | google-generative-ai | `https://generativelanguage.googleapis.com/v1beta` |
| `alibaba` | `qwen-portal` | 阿里百炼 | openai-completions | `https://dashscope.aliyuncs.com/compatible-mode/v1` |
| `mistral` | `mistral` | Mistral | openai-completions | `https://api.mistral.ai/v1` |
| `groq` | `groq` | Groq | openai-completions | `https://api.groq.com/openai/v1` |

PetClaw 套餐模型也由 Registry 或独立 `PetClawModelCatalog` 定义。`app_config` 只保存用户选择和套餐权益快照，不保存完整套餐模型 catalog。

### 5.6 API Key 安全机制

参考 LobsterAI 的 `${VAR}` 占位符模式：

```json
// openclaw.json 中写入
"providers": {
  "openai": {
    "baseUrl": "https://api.openai.com/v1",
    "api": "openai-completions",
    "apiKey": "${PETCLAW_APIKEY_OPENAI}",
    "auth": "api-key"
  }
}
```

```typescript
// collectSecretEnvVars() 返回
{ "PETCLAW_APIKEY_OPENAI": "sk-xxx..." }
```

Gateway 进程启动时注入这些环境变量，runtime 解析 `${VAR}` 占位符后使用真实值。

### 5.7 toOpenclawConfig()

```typescript
toOpenclawConfig() {
  const providers: Record<string, unknown> = {}
  for (const p of this.listProviders()) {
    if (!p.enabled) continue

    const definition = this.providerRegistry.get(p.id)
    const openClawProviderId = definition?.openClawProviderId ?? p.id
    const auth = definition?.auth ?? 'api-key'
    const apiKey = this.store.getApiKey(p.id)

    if (auth === 'api-key' && !apiKey) continue

    const envVar = `PETCLAW_APIKEY_${p.id.toUpperCase().replaceAll('-', '_')}`
    providers[openClawProviderId] = {
      baseUrl: p.baseUrl,
      api: p.apiFormat,
      ...(auth === 'api-key' ? { apiKey: `\${${envVar}}`, auth: 'api-key' } : {}),
      models: p.models.map(m => ({
        id: m.id,
        name: m.name,
        ...(m.reasoning && { reasoning: true }),
      }))
    }
  }
  return { mode: 'replace', providers }
}
```

默认模型引用也必须经过 Registry 转换：

```typescript
getDefaultOpenClawModelRef() {
  const pref = this.store.getModelPreference()
  const definition = this.providerRegistry.get(pref.defaultProviderId)
  const providerId = definition?.openClawProviderId ?? pref.defaultProviderId
  return `${providerId}/${pref.defaultModelId}`
}
```

---

## 6. SkillManager

### 6.1 文件位置

`src/main/skills/skill-manager.ts`

### 6.2 接口设计

```typescript
export interface Skill {
  id: string
  name: string
  description: string
  enabled: boolean
  isBuiltIn: boolean
  skillPath: string
  version?: string
}

export class SkillManager extends EventEmitter {
  constructor(private db: Database.Database, private skillsRoot: string) {}

  getSkillsRoot(): string
  async scan(): Promise<Skill[]>           // 扫描 skillsRoot 下的 SKILL.md
  setEnabled(id: string, enabled: boolean): void  // 状态存 kv 表
  list(): Skill[]
  getEnabled(): Skill[]

  toOpenclawConfig(): {
    entries: Record<string, { enabled: boolean }>
    load: { extraDirs: string[]; watch: boolean }
  }

  on('change', () => void): this
}
```

### 6.3 Skills 加载机制

通过 openclaw.json 的 `skills.load.extraDirs` 配置 Skills 目录，Openclaw runtime 原生扫描 SKILL.md：

```json
"skills": {
  "entries": {
    "web-search": { "enabled": true },
    "code-analyzer": { "enabled": false }
  },
  "load": {
    "extraDirs": ["{userData}/SKILLs"],
    "watch": true
  }
}
```

`entries` 控制每个 skill 的启用/禁用状态。`extraDirs` 告诉 runtime 去哪扫描。

### 6.4 enabled 状态存储

参考 LobsterAI，使用 kv 表存储 `skills_state` key，值为 `Record<string, boolean>` JSON。

---

## 7. McpManager

### 7.1 文件位置

`src/main/mcp/mcp-manager.ts`

### 7.2 接口设计

```typescript
export interface McpServer {
  id: string
  name: string
  description: string
  enabled: boolean
  transportType: 'stdio' | 'sse' | 'streamable-http'
  config: StdioConfig | HttpConfig
  createdAt: number
  updatedAt: number
}

export interface StdioConfig {
  command: string
  args: string[]
  env?: Record<string, string>
}

export interface HttpConfig {
  url: string
  headers?: Record<string, string>
}

export class McpManager extends EventEmitter {
  constructor(private db: Database.Database) {}

  create(server: Omit<McpServer, 'id' | 'createdAt' | 'updatedAt'>): McpServer
  update(id: string, patch: Partial<McpServer>): McpServer
  delete(id: string): void
  list(): McpServer[]
  get(id: string): McpServer | undefined
  setEnabled(id: string, enabled: boolean): void
  toOpenclawConfig(): OpenclawMcpConfig

  on('change', () => void): this
}
```

### 7.3 toOpenclawConfig()

MCP 服务器作为 plugin 注入 openclaw.json（参考 LobsterAI mcp-bridge 模式）：

```json
"plugins": {
  "entries": {
    "mcp-bridge": {
      "enabled": true,
      "config": {
        "servers": {
          "my-server": {
            "transport": "stdio",
            "command": "npx",
            "args": ["-y", "@modelcontextprotocol/server-filesystem"]
          }
        }
      }
    }
  }
}
```

---

## 8. MemorySearchConfigStore

### 8.1 文件位置

`src/main/memory/memory-search-config-store.ts`

### 8.2 工作原理

PetClaw 当前不提供前端直接读写 `MEMORY.md` 的独立记忆管理器。记忆相关配置以 `MemorySearchConfigStore` 为事实源，存入 `app_config.memorySearch.config`，由 `ConfigSync` 转换为 OpenClaw runtime 的 `memorySearch` 配置。

### 8.3 接口设计

```typescript
export type MemorySearchProvider = 'openai' | 'gemini' | 'voyage' | 'mistral' | 'ollama'

export interface MemorySearchConfig {
  enabled: boolean
  provider: MemorySearchProvider
  model: string
  remoteBaseUrl: string
  remoteApiKey: string
  vectorWeight: number
}

export class MemorySearchConfigStore {
  getConfig(): MemorySearchConfig
  setConfig(patch: Partial<MemorySearchConfig>): MemorySearchConfig
  toOpenclawConfig(): Record<string, unknown> | null
  collectSecretEnvVars(): Record<string, string>
}
```

### 8.4 敏感信息

`remoteApiKey` 可以存入本地 `app_config`，但写入 OpenClaw 配置时只能使用 `${PETCLAW_MEMORY_SEARCH_API_KEY}` 占位符；真实值通过 `collectSecretEnvVars()` 注入 runtime 环境。

---

## 9. ConfigSync 重构

### 9.1 从 deps 注入改为直接依赖

Phase 1 的 ConfigSync 使用 `ConfigSyncDeps` 接口注入，Phase 2 重构为直接接收 Manager 实例：

```typescript
export class ConfigSync {
  constructor(
    private engineManager: OpenclawEngineManager,
    private directoryManager: DirectoryManager,
    private modelRegistry: ModelRegistry,
    private skillManager: SkillManager,
    private mcpManager: McpManager,
    private imGatewayManager?: ImGatewayManager,
    private memorySearchConfigStore?: MemorySearchConfigStore,
  ) {}

  sync(reason: string): ConfigSyncResult {
    const config = {
      models: this.modelRegistry.toOpenclawConfig(),
      agents: {
        defaults: {
          timeoutSeconds: 3600,
          model: { primary: this.modelRegistry.getDefaultOpenClawModelRef() },
        },
        list: this.directoryManager.toOpenclawAgents(),
      },
      skills: this.skillManager.toOpenclawConfig(),
      mcp: this.mcpManager.toOpenclawConfig(),
      im: this.imGatewayManager?.toOpenclawConfig(),
      memorySearch: this.memorySearchConfigStore?.toOpenclawConfig(),
      hooks: { internal: { entries: { 'session-memory': { enabled: false } } } },
      gateway: this.preserveGatewayConfig(),
      commands: { ownerAllowFrom: ['gateway-client', '*'] },
    }
    this.atomicWrite(config)
    this.syncWorkspaceFiles()
    return { ok: true, changed: true, configPath: this.configPath }
  }

  collectSecretEnvVars(): Record<string, string> {
    return this.modelRegistry.collectSecretEnvVars()
  }
}
```

### 9.2 Workspace 文件同步

每次 `sync()` 时同步 workspace 下的 .md 文件：

**Main Agent**：只写 AGENTS.md（双区结构：用户区 + managed 区）

```
[用户自定义内容]

<!-- PetClaw managed: do not edit below this line -->

## System Prompt
<agent.systemPrompt>

<MANAGED_WEB_SEARCH_POLICY>
<MANAGED_EXEC_SAFETY>
<MANAGED_MEMORY_POLICY>
```

**目录 Agent**：目录本身作为 workspace，PetClaw 不主动写用户项目目录下的 `AGENTS.md`，避免污染用户仓库；目录对应的 `agent_id` 只写入 runtime 配置和本地数据库。

### 9.3 Manager change 事件自动触发 sync

```typescript
// index.ts 中注册
directoryManager.on('change', () => configSync.sync('directory-change'))
modelRegistry.on('change', () => configSync.sync('model-change'))
skillManager.on('change', () => configSync.sync('skill-change'))
mcpManager.on('change', () => configSync.sync('mcp-change'))
imGatewayManager.on('change', () => configSync.sync('im-change'))
```

---

## 10. CoworkSessionManager 升级

### 10.1 目录驱动的 session 归属

```typescript
createAndStart(title: string, cwd: string, prompt: string, opts: {
  useMainAgent?: boolean
  selectedModel?: { providerId: string; modelId: string }
  skillIds?: string[]
  origin?: 'chat' | 'im' | 'scheduler' | 'hook'
}): CoworkSession
```

SessionKey 格式：`agent:{agentId}:petclaw:{sessionId}`

`agentId` 不再由前端侧栏选择。主进程按目录上下文决定会话归属：

- `useMainAgent=true`：使用 `main`，workspace 指向 OpenClaw main agent 私有目录。
- `useMainAgent=false`：使用 `deriveAgentId(cwd)`，并通过 `DirectoryManager.ensureRegistered(cwd)` 幂等注册目录。
- `origin` 写入 `cowork_sessions.origin`，用于 UI 区分普通任务、IM 触发任务、定时任务运行和 hook 触发任务。

### 10.2 会话级模型覆盖

模型选择不作为 `chat.send` 参数传给 OpenClaw。ChatInputBox 必须始终有一个当前选中模型：优先取当前目录配置的模型覆盖，其次取 `app_config.model` 的全局默认模型。前端传给主进程的是 PetClaw 内部结构 `{ providerId, modelId }`，主进程通过 `ModelRegistry.toOpenClawModelRef()` 转成 OpenClaw model ref，再通过 `sessions.patch` 写入 OpenClaw session。

```typescript
const selectedModelRef = this.modelRegistry.toOpenClawModelRef(session.selectedModel)
await gateway.request('sessions.patch', {
  key: sessionKey,
  model: selectedModelRef,
})
```

模型优先级：

1. `session.selectedModel`：ChatInputBox 当前选中的模型，前端默认会填好。
2. `directory.modelOverride`：当前目录配置的模型覆盖，用于初始化 ChatInputBox。
3. `app_config.model.defaultProviderId/defaultModelId`：全局默认模型，用于没有目录覆盖时初始化 ChatInputBox。

`chat.send` 只发送 `sessionKey`、`message`、附件和幂等 key，不携带模型字段。OpenClaw 根据 `sessionKey` 找到已 patch 的 session 配置。`agents.defaults.model.primary` 仅作为非 UI 入口的运行时默认值；普通 UI 会话启动时必须 patch 明确模型。

### 10.3 outbound prompt 构建

发送消息时根据会话固化的 `directoryPath` / `agentId` 构建 prompt：

```typescript
private buildOutboundPrompt(session: Session, message: string): {
  sessionKey: string
  message: string
  workspace: string
  systemPrompt?: string
  activeSkillIds?: string[]
} {
  return {
    sessionKey: `agent:${session.agentId}:petclaw:${session.id}`,
    message,
    workspace: session.agentId === 'main'
      ? this.workspacePath
      : session.directoryPath,
    activeSkillIds: session.activeSkillIds,
  }
}
```

---

## 11. IPC 扩展

### 11.1 有效 IPC channels

所有 channel 遵循 `模块:动作` kebab-case 规范。

| Channel | 方向 | 说明 |
|---------|------|------|
| `cowork:session:start` | invoke | 创建并启动 Cowork 任务 |
| `cowork:session:continue` | invoke | 继续已有 Cowork 任务 |
| `cowork:session:list` | invoke | 获取任务列表 |
| `cowork:session:get` | invoke | 获取单个任务 |
| `cowork:session:stop` | invoke | 停止运行中的任务 |
| `cowork:session:delete` | invoke | 删除任务 |
| `directory:list` | invoke | 获取目录配置列表 |
| `directory:get` | invoke | 按 agentId 获取目录配置 |
| `directory:get-by-path` | invoke | 按路径获取目录配置 |
| `directory:update-name` | invoke | 更新目录别名 |
| `directory:update-model` | invoke | 更新目录模型覆盖 |
| `directory:update-skills` | invoke | 更新目录技能白名单 |
| `models:providers` | invoke | 获取所有 Provider |
| `models:provider` | invoke | 获取单个 Provider 配置 |
| `models:add-provider` | invoke | 添加 Provider |
| `models:update-provider` | invoke | 更新 Provider |
| `models:remove-provider` | invoke | 删除 Provider |
| `models:toggle-provider` | invoke | 启用/禁用 Provider |
| `models:default` | invoke | 获取全局默认模型 |
| `models:set-default` | invoke | 设置全局默认模型 |
| `models:set-api-key` | invoke | 写入 Provider API Key |
| `models:clear-api-key` | invoke | 清除 Provider API Key |
| `models:test-connection` | invoke | 测试 Provider 连接 |
| `models:add-model` | invoke | 添加 Provider 下的模型 |
| `models:remove-model` | invoke | 删除 Provider 下的模型 |
| `skills:list` | invoke | 获取所有 Skill |
| `skills:set-enabled` | invoke | 启用/禁用 Skill |
| `mcp:list` | invoke | 获取所有 MCP 服务器 |
| `mcp:create` | invoke | 创建 MCP 服务器 |
| `mcp:update` | invoke | 更新 MCP 服务器 |
| `mcp:delete` | invoke | 删除 MCP 服务器 |
| `im:load-config` | invoke | 获取 IM 实例配置 |
| `im:create-instance` | invoke | 创建 IM 平台实例 |
| `im:save-config` | invoke | 更新 IM 实例配置 |
| `im:delete-instance` | invoke | 删除 IM 实例 |
| `im:get-status` | invoke | 获取 IM 连接状态 |
| `im:set-binding` | invoke | 绑定 IM 对话到目录/agent |
| `im:status-update` | event | 推送 IM 连接状态变化 |

### 11.2 IPC 模块文件

| 文件 | 职责 |
|------|------|
| `ipc/chat-ipc.ts` | Cowork 任务启动、继续、停止、列表、删除 |
| `ipc/directory-ipc.ts` | 目录配置 handlers |
| `ipc/models-ipc.ts` | Model Provider handlers |
| `ipc/skills-ipc.ts` | Skill 管理 handlers |
| `ipc/mcp-ipc.ts` | MCP 服务器 handlers |
| `ipc/im-ipc.ts` | IM 实例、绑定和状态 handlers |

### 11.3 Preload 扩展

```typescript
// preload/index.ts 新增
directories: {
  list: () => ipcRenderer.invoke('directory:list'),
  get: (agentId: string) => ipcRenderer.invoke('directory:get', agentId),
  getByPath: (path: string) => ipcRenderer.invoke('directory:get-by-path', path),
  updateName: (agentId: string, name: string) => ipcRenderer.invoke('directory:update-name', agentId, name),
  updateModel: (agentId: string, model: string) => ipcRenderer.invoke('directory:update-model', agentId, model),
  updateSkills: (agentId: string, skillIds: string[]) => ipcRenderer.invoke('directory:update-skills', agentId, skillIds),
},
models: {
  providers: () => ipcRenderer.invoke('models:providers'),
  provider: (id: string) => ipcRenderer.invoke('models:provider', id),
  addProvider: (data: unknown) => ipcRenderer.invoke('models:add-provider', data),
  updateProvider: (id: string, patch: unknown) => ipcRenderer.invoke('models:update-provider', id, patch),
  removeProvider: (id: string) => ipcRenderer.invoke('models:remove-provider', id),
  toggleProvider: (id: string, enabled: boolean) => ipcRenderer.invoke('models:toggle-provider', id, enabled),
  defaultModel: () => ipcRenderer.invoke('models:default'),
  setDefaultModel: (model: unknown) => ipcRenderer.invoke('models:set-default', model),
  setApiKey: (providerId: string, apiKey: string) => ipcRenderer.invoke('models:set-api-key', providerId, apiKey),
  clearApiKey: (providerId: string) => ipcRenderer.invoke('models:clear-api-key', providerId),
  testConnection: (id: string) => ipcRenderer.invoke('models:test-connection', id),
  addModel: (providerId: string, model: unknown) => ipcRenderer.invoke('models:add-model', providerId, model),
  removeModel: (providerId: string, modelId: string) => ipcRenderer.invoke('models:remove-model', providerId, modelId),
},
skills: {
  list: () => ipcRenderer.invoke('skills:list'),
  setEnabled: (id: string, enabled: boolean) => ipcRenderer.invoke('skills:set-enabled', id, enabled),
},
mcp: {
  list: () => ipcRenderer.invoke('mcp:list'),
  create: (data: unknown) => ipcRenderer.invoke('mcp:create', data),
  update: (id: string, patch: unknown) => ipcRenderer.invoke('mcp:update', id, patch),
  delete: (id: string) => ipcRenderer.invoke('mcp:delete', id),
},
im: {
  listInstances: () => ipcRenderer.invoke('im:load-config'),
  createInstance: (platform: string, credentials: Record<string, unknown>, name?: string) =>
    ipcRenderer.invoke('im:create-instance', platform, credentials, name),
  updateInstance: (id: string, patch: Record<string, unknown>) =>
    ipcRenderer.invoke('im:save-config', id, patch),
  deleteInstance: (id: string) => ipcRenderer.invoke('im:delete-instance', id),
  getStatus: () => ipcRenderer.invoke('im:get-status'),
  setBinding: (conversationId: string, instanceId: string, peerKind: 'dm' | 'group', directoryPath: string, agentId: string) =>
    ipcRenderer.invoke('im:set-binding', conversationId, instanceId, peerKind, directoryPath, agentId),
},
```

### 11.4 IM 实例数量约束

`im:create-instance` 不能只依赖前端隐藏按钮，主进程必须按平台能力做后端校验：

```typescript
const MULTI_INSTANCE_PLATFORMS = ['dingtalk', 'feishu', 'wecom'] as const

const PLATFORM_INFO = {
  feishu: { maxInstances: 3 },
  dingtalk: { maxInstances: 3 },
  wecom: { maxInstances: 3 },
  wechat: { maxInstances: 1 },
}
```

创建实例时：
- 先统计 `im_instances` 中同平台现有实例数量。
- 如果数量已达到 `PLATFORM_INFO[platform].maxInstances`，返回用户可见错误，不写数据库。
- 微信是单实例平台，只允许存在一个 `wechat` 实例；UI 的“已登录账号”只更新这一条实例的 credentials/name/status，不创建第二条。
- 飞书、钉钉、企微是多实例平台，允许多个 bot 实例，但仍受 `maxInstances=3` 限制。

这样做的原因是频道侧栏的层级依赖后端能力：多实例平台展示 `平台 → 实例 → 对话`，微信展示 `平台 → 对话`。如果后端允许多个微信实例，侧栏和绑定模型会被迫出现不存在的“微信实例层”。

---

## 12. 主页布局（任务中心 + 频道分组）

参考方向：LobsterAI 侧栏的信息密度与任务中心心智，但不继续沿用旧的 Agent-centric UI。当前有效架构是 Directory-driven Agent：用户看见的是任务、频道、定时任务和工作目录上下文；`agentId` 只作为运行时路由字段存在。

### 12.1 整体结构 — 三栏布局

PetClaw Chat 窗口采用**左侧栏 + 中间主区域 + 可选右面板**三栏布局。

```
左侧栏
  新任务
  技能
  专家套件
  定时任务
  IM 频道

  [任务] [频道]

  草稿
    未发送/输入中的任务

  定时任务            [折叠按钮]
    [折叠按钮] 定时任务1
      消息1
      消息2
    [折叠按钮] 定时任务2
      消息1
      消息2

  任务
    普通 Cowork session
```

**三栏职责**：
- **左侧栏**（建议 `w-[260px]`）：任务导航、频道分组、草稿、定时任务分组、普通任务列表、用户/设置。
- **主内容区**（`flex-1`）：根据 `activeView` 切换聊天、技能、专家套件、定时任务、IM 频道和设置入口。
- **右面板**（建议 `w-[280px]`）：当前任务的执行状态和上下文摘要，显示待办、产物、工作目录、技能、MCP 和权限审批。

**关键原则**：
- 左侧栏不显式展示“工作目录”分组，也不让用户直接管理 Agent。
- 工作目录只在输入框、任务详情、非默认目录任务 metadata 和右侧任务监控中出现。
- `任务` 是 Cowork session 的产品名称，但左侧栏任务列表只展示用户主动发起的普通任务。
- `频道` 是 IM 来源会话的分组视图，按平台实例和对话分组，不是工作目录下的子导航。
- 定时任务运行 session 挂在定时任务分组下，不混入普通任务列表。

### 12.2 页面路由（ViewType）

```typescript
type ViewType = 'chat' | 'skills' | 'expert-kits' | 'cron' | 'im-channels' | 'settings'
```

每个侧栏元素与主内容区的映射关系：

| 侧栏操作 | 主内容区变化 | 侧栏列表变化 | 状态变更 |
|----------|------------|------------|---------|
| **`+ 新任务`** | 切换到 `chat` → 新任务输入态 | 草稿区出现或聚焦当前草稿 | `activeView='chat'`, `activeSessionId=null` |
| **技能** | 切换到 `skills` → 技能管理页 | 保持当前列表 | `activeView='skills'` |
| **专家套件** | 切换到 `expert-kits` → 专家套件页 | 保持当前列表 | `activeView='expert-kits'` |
| **定时任务** | 切换到 `cron` → 定时任务管理页 | 保持当前列表 | `activeView='cron'` |
| **IM 频道** | 切换到 `im-channels` → IM 实例与对话绑定管理页 | 保持当前 `sidebarTab` 不变 | `activeView='im-channels'` |
| **任务列表项** | 切换到 `chat` → 加载该 Cowork session | 该任务高亮 | `activeSessionId=session.id` |
| **频道列表项** | 切换到 `chat` → 加载该频道映射的 Cowork session；无映射时显示频道详情 | 该频道高亮 | `activeSessionId=mapping.sessionId ?? null` |
| **设置齿轮** | 切换到 `settings` → 设置全页面 | 无变化 | `activeView='settings'` |

### 12.3 核心状态模型

```typescript
interface WorkbenchState {
  activeView: ViewType
  previousView: ViewType
  activeSessionId: string | null
  activeDraftId: string | null
  sidebarTab: 'tasks' | 'channels'
  collapsedScheduledTaskIds: string[]
  collapsedChannelGroupIds: string[]
  taskFilter: 'all' | 'chat' | 'im' | 'scheduler'
  channelFilter: 'all' | 'wechat' | 'wecom' | 'dingtalk' | 'feishu'
  taskMonitorOpen: boolean
  settingsTab: string
}
```

`agentId`、`directoryPath`、`origin` 都是任务或频道记录的 metadata，不再作为左侧栏主状态。新任务发送时由 `cwd` 和 `useMainAgent` 决定运行时 agent；历史任务继续读取 `cowork_sessions.agent_id`。

### 12.4 侧栏分区

| 分区 | 内容 | 数据来源 | 行为 |
|------|------|----------|------|
| **主导航** | 新任务、技能、专家套件、定时任务、IM 频道 | 静态路由 + feature flag | 点击切换主内容区 |
| **Tab 切换** | `任务` / `频道` | 本地 UI 状态 | 只切换侧栏列表，不强制改变主内容区 |
| **草稿** | 未发送或输入中的任务 | renderer draft store；后续可持久化 | 点击恢复输入框内容 |
| **定时任务分组** | 每个 scheduled task 一组，可折叠展示最近运行消息；无定时任务时隐藏整个分组 | `scheduler:list` + `scheduler:list-runs` + `scheduled_task_meta` | 点击任务名进定时任务详情；点击运行消息进 Cowork session |
| **任务列表** | 用户主动发起的普通 Cowork session | `cowork_sessions.origin = 'chat'` + `directory_path` | 点击进入聊天页 |
| **频道列表** | 按 IM 平台实例与对话分组 | `im_instances` + `im_conversation_bindings` + `im_session_mappings` | 点击进入频道会话或频道详情 |
| **底部栏** | 用户、套餐、设置 | app_config / 用户资料 | 齿轮进入 Settings |

侧栏视觉权重以任务为主。工作目录只在非默认目录任务中用弱 metadata 展示，例如 `petclaw · 2分钟前`，不作为分组标题。

### 12.5 任务 Tab — 列表结构

`任务` tab 展示当前用户主动发起、需要继续处理或回看的普通 Cowork session。

```
草稿
  未发送：帮我整理 PRD...

定时任务
  ▸ 每日资讯
  ▾ 周报生成
    本周进度摘要
    风险总结

任务
  帮我创建 Git 提交消息...
    2分钟前
  重构 PetClaw 侧栏
    petclaw · 18分钟前
```

**普通任务**：
- 来源：`cowork_sessions.origin = 'chat'`
- 主要展示：title、更新时间；默认 Main 工作区不展示，非默认目录才以弱 metadata 展示目录短名。

**IM 触发任务**：
- 来源：`cowork_sessions.origin = 'im'` 或 `im_session_mappings.session_id`
- 展示位置：频道 Tab，按平台、实例和对话分组。
- 主要展示：平台/实例/对话名称、最后消息摘要、绑定目录短名。

**定时任务运行任务**：
- 来源：`cowork_sessions.origin = 'scheduler'` 或 `scheduled_task_meta.task_id` 对应的 run session
- 展示位置：定时任务分组，挂在对应 scheduled task 下；没有 scheduled task 时隐藏整个分组。
- 主要展示：定时任务名称、运行时间、运行状态、绑定目录短名。

列表排序默认按 `updated_at` 倒序。置顶、运行中、草稿优先显示；折叠分组不改变底层排序，只改变展示。

### 12.6 定时任务分组

定时任务在侧栏内有独立分组，因为它既是管理对象，也是会产生 Cowork session 的任务来源。

```
定时任务            [折叠按钮]
  ▾ 每日 Git 摘要
    09:00 成功：生成提交摘要
    昨天 09:00 失败：目录不存在
  ▸ 每周周报
```

数据组合：
- `scheduler:list` 提供定时任务本体、启用状态和下一次运行时间。
- `scheduler:list-runs` 或 `scheduler:list-all-runs` 提供最近运行记录。
- `scheduled_task_meta` 提供 `directory_path`、`agent_id`、`origin`、`binding`。
- Cowork session 由运行记录中的 `sessionId` 或后续关联表定位。

交互：
- 点击分组标题：展开/折叠该定时任务的最近运行消息。
- 点击定时任务名称：打开 `cron` 页面并定位到任务详情。
- 点击运行消息：进入对应 Cowork session。
- 运行中任务显示进行中状态，并在右侧任务监控中显示待办和产物。

### 12.7 频道 Tab — 分组设计

`频道` tab 不是“目录下的频道”，而是 IM 入口的会话视图。后端支持部分平台多实例，因此多实例平台按平台实例分组，微信这类单实例平台直接按对话分组。

```
频道
  飞书
    ▾ 产品群 Bot
      产品周会
        petclaw · 群聊 · 12分钟前
      张三
        Main · 私聊 · 昨天
    ▸ 研发群 Bot

  钉钉
    ▾ 客服 Bot
      客户 A 项目群
        test · 群聊 · 3小时前

  企业微信
    ▸ 内部助手

  微信 · 已登录：徐旭
    李四
      Main · 私聊 · 1周前
    家庭群
      petclaw · 群聊 · 昨天
```

分组层级：
1. **平台**：`wechat`、`wecom`、`dingtalk`、`feishu`。
2. **实例**：仅多实例平台展示该层级。飞书、钉钉、企微按 `im_instances` 展示多个实例；微信当前单实例，不展示实例节点。
3. **对话**：`im_conversation_bindings` 和 `im_session_mappings` 中出现过的 `conversation_id`。微信直接在平台下展示对话。
4. **会话**：该对话映射到的 Cowork session。

字段含义：
- `im_instances.directory_path` 是实例级默认工作目录；未设置时显示 `Main`。
- 微信账号信息只作为平台标题 metadata，例如 `微信 · 已登录：徐旭`，不作为可展开实例层级。
- `im_conversation_bindings.directory_path` 是对话级覆盖，优先展示。
- `im_session_mappings.session_id` 让同一 IM 对话复用同一 Cowork session。
- `peer_kind` 决定文案：`dm` 显示私聊，`group` 显示群聊。

交互：
- 点击平台或实例标题：展开/折叠；微信只有平台标题可折叠。
- 点击对话行：如果有 `session_id`，进入该 Cowork session；如果没有，打开频道详情/绑定页。
- 对话行右侧可提供“绑定工作目录”入口，写入 `im_conversation_bindings` 并触发 ConfigSync。
- 顶部主导航的 `IM 频道` 进入完整管理页；侧栏 `频道` tab 是日常会话入口。

### 12.8 `+ 新任务` — 创建流程

```
用户点击「+ 新任务」
  │
  ├→ 1. activeView = 'chat'
  ├→ 2. activeSessionId = null
  ├→ 3. activeDraftId 指向当前草稿
  └→ 4. 用户在输入框输入消息并发送
       │
       ├→ cowork:session:start({ prompt, cwd, skillIds, selectedModel, ... })
       ├→ CoworkSessionManager 根据 cwd/useMainAgent 确定 agentId
       ├→ 创建 cowork_sessions，origin='chat'
       ├→ activeSessionId = sessionId
       └→ 草稿移除，任务列表顶部新增该任务
```

**注意**：「+ 新任务」不立刻创建 DB 会话，只创建或聚焦草稿。真正的 Cowork session 在用户发送第一条消息时创建。

### 12.9 欢迎页（空状态）

当 `activeView === 'chat' && activeSessionId === null` 时显示欢迎页：

```
                      🐾

               不止聊天，搞定一切

        本地运行，自主规划，安全可控的 AI 工作搭子

     ┌─────────────┐  ┌─────────────┐  ┌─────────────┐
     │ 📁           │  │ ✍️           │  │ 📊           │
     │ 文件整理      │  │ 内容创作      │  │ 文档处理      │
     │ 智能整理和管  │  │ 创作演义文稿  │  │ 处理和分析文  │
     │ 理本地文件    │  │ 和多种内容    │  │ 档数据内容    │
     └─────────────┘  └─────────────┘  └─────────────┘

     ┌──────────────────────────────────────────────┐
     │ 描述任务，/ 调用技能与工具                      │
     ├──────────────────────────────────────────────┤
     │ [📁 cwd] [🔧] [🔌] [📎]      [⚡ 标准] [↑]   │
     └──────────────────────────────────────────────┘
```

**快捷卡片**：3 张功能引导卡片（`rounded-[14px]`，`shadow-[var(--shadow-card)]`），点击后填充 prompt 到输入框并自动发送：
- **文件整理** — icon: `FolderOpen` — "帮我整理桌面文件，按类型分类到对应文件夹"
- **内容创作** — icon: `PenLine` — "帮我写一篇关于…的文章"
- **文档处理** — icon: `BarChart3` — "帮我分析这份文档的关键信息"

### 12.10 聊天页

当 `activeView === 'chat' && activeSessionId !== null` 时显示聊天页：

**聊天顶栏**（`ChatHeader`）：
```
┌──────────────────────────────────────────────────┐
│  会话标题                      [问题反馈]  [≡]    │
└──────────────────────────────────────────────────┘
```
- 左侧：当前会话标题（可编辑，blur 时保存）
- 右侧：`问题反馈` 链接 + `≡` 右面板 toggle

**消息区域**支持：
- 用户消息（右对齐气泡，`bg-bg-bubble-user`）
- AI 消息（左对齐 + PetClaw 头像，`bg-bg-bubble-ai`，Markdown 渲染）
- 工具调用展示（折叠面板，显示工具名 + 输入/输出摘要）
- 权限审批卡片（允许/拒绝按钮）
- 流式输出：打字机效果 + typing indicator

**消息数据来源**：
```
activeSessionId 变化
  → window.api.cowork.session(id) → 获取会话元数据
  → CoworkStore.getMessages(id) → 获取历史消息
  → window.api.cowork.onMessage → 监听新增消息（流式）
  → window.api.cowork.onMessageUpdate → 监听消息更新（流式追加）
  → window.api.cowork.onComplete → 会话完成
  → window.api.cowork.onError → 错误处理
```

### 12.11 技能页、专家套件页、定时任务页和 IM 频道页

| 页面 | ViewType | 触发 | 内容 | Phase |
|------|----------|------|------|-------|
| 技能管理 | `skills` | 侧栏「技能」 | Skill 列表 + 开关切换 + 描述 | Phase 2 |
| 专家套件 | `expert-kits` | 侧栏「专家套件」 | 按场景组合 skills、模型和推荐 prompt | 后续可按 feature flag 开启 |
| 定时任务 | `cron` | 侧栏「定时任务」 | 定时任务列表 + 创建/编辑/删除 + 运行历史 | Phase 3 |
| IM 频道 | `im-channels` | 侧栏「IM 频道」 | IM 实例、对话绑定、连接状态和最近会话 | Phase 3 |

**技能页布局**：
```
┌──────────────────────────────────────┐
│  技能管理                             │
│                                      │
│  ┌──────────────────────────────┐    │
│  │ 🔍 搜索技能...                │    │
│  └──────────────────────────────┘    │
│                                      │
│  ┌──────────────────────────────┐    │
│  │ web-search        [开关 ☑]    │    │
│  │ 搜索互联网获取最新信息         │    │
│  ├──────────────────────────────┤    │
│  │ code-analyzer     [开关 ☑]    │    │
│  │ 分析和优化代码                 │    │
│  ├──────────────────────────────┤    │
│  │ file-manager      [开关 ☐]    │    │
│  │ 管理本地文件和目录             │    │
│  └──────────────────────────────┘    │
└──────────────────────────────────────┘
```

数据来源：`window.api.skills.list()` → 列表渲染，`window.api.skills.setEnabled(id, bool)` → 开关切换

### 12.12 右面板 — 任务监控（TaskMonitor）

参考 LobsterAI `主页3.png` 右侧面板，仅在 `activeView === 'chat'` 时可展开：

```
┌──────────────┐
│  任务监控   ☐ │
│              │
│  待办         │
│  暂无待办     │
│              │
│  产物         │
│  📁 ~/Desktop │
│              │
│  技能与 MCP   │
│  · web-search │
│  · 文件服务器  │
└──────────────┘
```

| 区域 | 数据来源 | 更新时机 |
|------|----------|----------|
| **待办** | 解析当前会话 AI 消息中的 TODO/checkbox | 消息更新时 |
| **产物** | 当前会话的 cwd | 会话加载 / CwdSelector 切换时 |
| **技能与 MCP** | 当前会话启用的 skillIds + 全局 MCP 列表 | 会话加载时 |

**toggle 行为**：
- 聊天顶栏 `≡` 按钮点击 → `taskMonitorOpen = !taskMonitorOpen`
- 收起时主内容区 `flex-1` 占满，展开时右面板 `w-[240px]` 从右侧推入
- 非 chat view 时自动收起
- Phase 2：先做面板骨架 + 产物/技能展示，待办解析放 Phase 3

### 12.13 连接器弹窗（Connector）

参考 LobsterAI `主页3.png` 连接器弹窗，PetClaw 用于 MCP 服务器快捷管理：

```
┌─────────────────────────────────┐
│  连接器  Beta                    │
│  在此快速开关，完整选项在设置中管理 │
│                                 │
│  ☐ 我的浏览器                    │
│  ☐ 提醒事项                      │
│  ☑ 日历                    ×     │
│  ☐ 备忘录                        │
│  ☐ 邮件                          │
│  ☐ 通讯录                        │
│                                 │
│  打开连接器完整设置               │
└─────────────────────────────────┘
```

**数据流**：
```
打开连接器弹窗
  → window.api.mcp.list() → 获取 MCP 服务器列表
  → 每行渲染：服务器名 + enabled toggle
  → toggle 切换 → window.api.mcp.setEnabled(id, bool)
  → McpManager.setEnabled() → emit('change') → ConfigSync.sync()
  → 底部「打开连接器完整设置」→ activeView='settings', settingsTab='connectors'
```

**触发方式**：
- 输入框工具栏「🔌」连接器图标点击
- Settings 连接器页的快捷入口

侧栏导航「IM 频道」进入 `im-channels` 管理页，不再打开连接器弹窗。连接器弹窗只负责 MCP/外部工具的快捷开关；IM 平台实例、对话绑定和多实例状态在 IM 频道页内管理。

---

## 13. Settings 页面（全页面）

参考设计稿：`docs/设计/设置页面.png`

### 13.1 设计变更

Settings 从**弹窗 Modal** 改为**独立全页面**，作为 `ViewType = 'settings'` 路由。页面采用**左侧分类菜单 + 右侧内容区**布局，顶部有「← 返回应用」按钮返回主界面。

```
┌──────────────────────────────────────────────────────────────────┐
│  ← 返回应用                                                      │
│                                                                  │
│  通用                        偏好设置                             │
│  ◉ 偏好设置                  语言、主题、字体与面板布局等个性化偏好   │
│  👤 个人资料                                                      │
│  ℹ️ 关于                     ┌──────────────────────────────────┐ │
│                              │  语言                             │ │
│  AI 配置                     │  选择界面语言            中文 ∨    │ │
│  ⚙️ Agent 引擎               ├──────────────────────────────────┤ │
│  🧠 模型                     │  主题亮暗                         │ │
│  🤖 Agent                    │  浅色、深色，或跟随系统   亮色 ∨    │ │
│  📝 记忆                     ├──────────────────────────────────┤ │
│                              │  对话字号                         │ │
│  扩展与集成                   │  调整对话中的文字大小      小 ∨    │ │
│  🔌 连接器                    └──────────────────────────────────┘ │
│  🔧 MCP 服务                                                     │
│                              ┌──────────────────────────────────┐ │
│                              │  任务面板位置                      │ │
│                              │  任务面板在工作区中的展示位置       │ │
│                              │                     固定在右侧 ∨  │ │
│                              └──────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────┘
       240px                            flex-1
```

### 13.2 页面路由

```typescript
// ViewType 更新
type ViewType = 'chat' | 'skills' | 'cron' | 'settings'
```

**进入方式**：侧栏底部齿轮图标点击 → `activeView = 'settings'`
**返回方式**：点击「← 返回应用」→ `activeView = 上次非 settings 的 view`（通常是 'chat'）

进入 Settings 页面时，主侧栏（Sidebar）**隐藏**，由 Settings 自身的左侧菜单替代。返回时恢复主侧栏。

### 13.3 左侧分类菜单

参考 LobsterAI 设置页面的三段式分类结构：

| 分类 | 菜单项 | 组件 | 内容 |
|------|--------|------|------|
| **通用** | 偏好设置 | `PreferenceSettings` | 语言、主题亮暗、对话字号、任务面板位置 |
| | 个人资料 | `ProfileSettings` | 昵称、头像、职业角色（来自 Onboarding） |
| | 关于 | `AboutSettings` | 版本号、开源协议、反馈链接、更新检查 |
| **AI 配置** | Agent 引擎 | `EngineSettings` | Openclaw 引擎状态、版本、重启按钮、日志查看 |
| | 模型 | `ModelSettings` | Provider 列表（启用/禁用）+ API Key + Base URL + API 格式 + 测试连接 + 可用模型列表 + 自定义 Provider |
| | 工作目录 | `DirectorySettings` | 目录列表 + 别名 + 模型覆盖 + 技能白名单 |
| | 记忆 | `MemorySettings` | MEMORY.md 内容查看/编辑 + 搜索 |
| **扩展与集成** | 连接器 | `ConnectorSettings` | MCP/IM 服务快捷开关（同连接器弹窗，但完整版） |
| | MCP 服务 | `McpSettings` | MCP 服务器详细管理（添加/编辑/删除 + 传输协议选择 + 配置编辑） |

### 13.4 右侧内容区 — 卡片分组设计

参考 LobsterAI 设置页面，右侧内容区采用**卡片分组**布局：

**卡片结构**：
- 每个卡片是一组相关设置项，圆角 `rounded-[14px]`，`bg-bg-card border border-border`
- 卡片内多个设置项用分隔线 `border-b border-border` 分隔
- 最后一个设置项无分隔线

**设置项行结构**：
```
┌────────────────────────────────────────┐
│  标题                                  │
│  描述文字（灰色小字）        [控件 ∨]   │
├────────────────────────────────────────┤
│  标题                                  │
│  描述文字                    [控件 ∨]   │
└────────────────────────────────────────┘
```

- 左侧：设置项标题（`text-[14px] font-medium`）+ 描述（`text-[13px] text-text-tertiary`）
- 右侧：控件（下拉选择 / 开关 / 输入框 / 按钮）
- 行高度：自适应内容，内边距 `py-4 px-5`

**页面标题区**：
- 标题：`text-[24px] font-bold`（如「偏好设置」）
- 副标题：`text-[14px] text-text-tertiary`（如「语言、主题、字体与面板布局等个性化偏好」）
- 标题区与第一个卡片间距 `mb-6`

### 13.5 各页面设置项详细

**偏好设置**（PreferenceSettings）：

| 卡片 | 设置项 | 控件类型 | 选项 |
|------|--------|----------|------|
| 外观 | 语言 | 下拉 | 中文 / English |
| | 主题亮暗 | 下拉 | 亮色 / 深色 / 跟随系统 |
| | 对话字号 | 下拉 | 小 / 中 / 大 |
| 布局 | 任务面板位置 | 下拉 | 固定在右侧 / 隐藏 |
| 快捷键 | 打开聊天 | 快捷键录入 | 默认 `Cmd+Shift+P` |
| | 语音输入 | 快捷键录入 | 自定义 |

**模型**（ModelSettings）— 特殊两栏布局：

```
┌──────────────────────────────────────────────────────────┐
│  模型                                              ✕    │
│                                                         │
│  ┌──────────────┐  ┌──────────────────────────────────┐ │
│  │ Provider 列表 │  │  DeepSeek 提供商设置 ✎    [已开启]│ │
│  │              │  │                                  │ │
│  │ 🟢 Zhipu    │  │  API Key                         │ │
│  │ 🔵 MiniMax  │  │  [设定 API Key +          ] [👁]  │ │
│  │ 🟣 Volceng  │  │                                  │ │
│  │ 🟡 Youdao   │  │  API Base URL                    │ │
│  │ 🔴 Qianfan  │  │  [https://api.deepseek.com/v1 ]  │ │
│  │ ⚡ StepFun  │  │                                  │ │
│  │ 🟠 Xiaomi   │  │  API 格式                        │ │
│  │ 🐙 Ollama   │  │  ○ Anthropic 兼容  ● OpenAI 兼容  │ │
│  │              │  │  请选择 API 协议兼容格式           │ │
│  │              │  │                                  │ │
│  │              │  │  ↔ 测试连接                      │ │
│  │              │  │                                  │ │
│  │              │  │  可用模型列表            [+ 添加]  │ │
│  │              │  │  ┌────────────────────────────┐  │ │
│  │              │  │  │ ◉ DeepSeek Reasoner        │  │ │
│  │              │  │  │   deepseek-reasoner         │  │ │
│  │              │  │  └────────────────────────────┘  │ │
│  │ [+ 添加自定义]│  │                                  │ │
│  └──────────────┘  └──────────────────────────────────┘ │
│                                         [取消] [保存]    │
└──────────────────────────────────────────────────────────┘
```

自定义 Provider 额外显示「显示名称」字段：

```
┌──────────────────────────────────────────────────────────┐
│  Custom0 提供商设置                           [未开启]    │
│                                                         │
│  API Key                                                │
│  [输入你的 API Key                              ]       │
│                                                         │
│  显示名称                                                │
│  [输入自定义名称...                             ]       │
│                                                         │
│  API Base URL                                           │
│  [输入 API 基础 URL                             ]       │
│  Anthropic 兼容 (已是默认加为): https://api.xxx...       │
│  OpenAI 兼容 (已是默认加为): https://api.xxx...          │
│                                                         │
│  API 格式                                                │
│  ○ Anthropic 兼容  ○ OpenAI 兼容                         │
│  请选择 API 协议兼容格式                                  │
│                                                         │
│  ↔ 测试连接                                              │
│                                                         │
│  可用模型列表                                   [+ 添加] │
│  （添加一个模型）                                         │
└──────────────────────────────────────────────────────────┘
```

**Provider 列表**：

- 左侧固定宽 `w-[200px]`，每行 = 品牌 Logo + 名称 + 启用状态圆点（绿 = 已启用、灰 = 未启用）
- 选中态：`bg-bg-active rounded-[10px]`
- 底部「+ 添加自定义」按钮，点击新建空白 Custom Provider 并自动选中
- 预设 Provider 列表：PetClaw（内置）、OpenAI、Anthropic、DeepSeek、Zhipu、MiniMax、Volcengine、Youdao、Qianfan、StepFun、Xiaomi、Ollama

**右侧配置面板**（选中 Provider 后显示）：

| 字段 | 控件 | 说明 |
|------|------|------|
| 标题栏 | 文本 + 编辑图标 + 开启/关闭 Toggle | Provider 名称可编辑（仅自定义），Toggle 控制是否启用 |
| API Key | 密码输入框 + 显示/隐藏按钮 | 预设 Provider 显示「设定 API Key +」占位；PetClaw 内置无此项 |
| 显示名称 | 文本输入框 | **仅自定义 Provider**，用于侧栏和选择器中的显示名 |
| API Base URL | 文本输入框 | 预填默认值；自定义 Provider 下方显示各格式的默认地址提示 |
| API 格式 | Radio 两选一 | `Anthropic 兼容` / `OpenAI 兼容`，影响请求协议 |
| 测试连接 | 按钮 + 状态 | `↔ 测试连接`，成功显示 `✅ 连接成功 (120ms)`，失败显示 `❌ 连接失败` + 错误信息 |
| 可用模型列表 | 列表 + 添加按钮 | 显示该 Provider 下可用的模型，每行 = 模型显示名 + 模型 ID（灰字）；右上角「+ 添加模型」 |

**添加模型弹窗**（点击「+ 添加模型」后，居中 Modal）：

```
┌────────────────────────────────────┐
│  添加新模型                    ✕   │
│                                    │
│  模型名称                          │
│  ┌──────────────────────────────┐  │
│  │ GPT-4                        │  │
│  └──────────────────────────────┘  │
│                                    │
│  模型ID                            │
│  ┌──────────────────────────────┐  │
│  │ gpt-4                        │  │
│  └──────────────────────────────┘  │
│                                    │
│  ☑ 支持图像输入                     │
│                                    │
│                    [取消]  [保存]   │
└────────────────────────────────────┘
```

| 字段 | 控件 | 说明 |
|------|------|------|
| 模型名称 | 文本输入框 | 显示名称，如 `GPT-4`、`DeepSeek Reasoner` |
| 模型ID | 文本输入框 | API 调用使用的模型标识，如 `gpt-4`、`deepseek-reasoner` |
| 支持图像输入 | Checkbox | 标记该模型是否支持多模态图像输入 |

UI 规范：
- Modal 背景 `bg-bg-card rounded-[14px] border border-border`，`max-w-[420px]`
- 标题 `text-[18px] font-semibold`，右上角 `✕` 关闭按钮
- 输入框 `rounded-[10px] bg-bg-input border-none px-4 py-3 text-[14px]`
- Checkbox 使用 `accent-primary rounded-[4px]`，label `text-[14px] text-text-secondary`
- 按钮区右对齐：「取消」ghost 按钮 + 「保存」primary 按钮，间距 `gap-3`
- 交互：`active:scale-[0.96] duration-[120ms]`

**操作按钮**：底部右对齐「取消」（ghost）+「保存」（primary），保存触发 `window.api.models.updateProvider(providerId, config)` → ModelRegistry.update() → ConfigSync.sync()。

### 13.6 UI 规范

遵循 PetClaw 设计系统 + 参考 LobsterAI 设置页风格：

- **返回按钮**：`← 返回应用`，`text-[14px] text-text-secondary hover:text-text-primary`
- **左侧菜单**：`w-[240px]` 固定，分类标题 `text-[12px] text-text-tertiary font-medium uppercase tracking-wider`
- **菜单项**：`rounded-[10px]` 选中态 `bg-bg-active font-medium`，未选 `hover:bg-bg-hover`
- **卡片**：`rounded-[14px] bg-bg-card border border-border`，内边距 `p-0`（由行自身管理 padding）
- **分隔线**：`mx-5 border-b border-border`（卡片内行之间）
- **下拉控件**：`rounded-[10px] border border-border px-3 py-1.5 text-[13px]`，右侧 chevron 图标
- **交互**：`active:scale-[0.96] duration-[120ms]`

### 13.7 数据流

```
Settings 页面加载
  ├→ 偏好设置：window.api.getSetting('theme') / 'language' / 'fontSize' / ...
  ├→ 模型：window.api.models.providers() → 渲染 Provider 列表
  ├→ 工作目录：window.api.directories.list() → 渲染目录列表
  ├→ 记忆：window.api.memory.read(workspace) → 渲染 MEMORY.md
  ├→ MCP：window.api.mcp.list() → 渲染 MCP 服务器列表
  └→ 引擎：window.api.engine.onStatus() → 实时引擎状态

设置项变更
  → window.api.setSetting(key, value) → kvSet(db, key, value)
  → 需要同步 openclaw.json 的变更（目录/模型/Skill/MCP/IM）
    → Manager.update() → emit('change') → ConfigSync.sync()

模型设置 — 详细数据流
  加载
    → window.api.models.providers() → ModelRegistry.getProviders()
    → 返回 Provider[] 含 { id, name, logo, enabled, apiFormat, models[] }
  选中 Provider
    → window.api.models.provider(id) → ModelRegistry.getProvider(id)
    → 返回完整配置（API Key 返回 masked 值 sk-****xxx）
  保存 Provider
    → window.api.models.updateProvider(id, config) → ModelRegistry.updateProvider()
    → API Key 通过 kvSet(db, `apikey:${providerId}`, encrypted) 安全存储
    → ConfigSync.sync() 写入 openclaw.json（Key 用 ${PETCLAW_APIKEY_xxx} 占位符）
  启用/禁用 Provider
    → window.api.models.toggleProvider(id, enabled) → ModelRegistry.toggleProvider()
    → ConfigSync.sync()
  添加自定义 Provider
    → window.api.models.addCustomProvider(config) → ModelRegistry.addCustomProvider()
    → 返回新 providerId
  删除自定义 Provider
    → window.api.models.deleteProvider(id) → ModelRegistry.deleteProvider()
    → 仅自定义可删除，预设不可删
  测试连接
    → window.api.models.testConnection(id) → ModelRegistry.testConnection()
    → 使用该 Provider 的 API Key + Base URL 发一个轻量请求
    → 返回 { success: boolean, latencyMs: number, error?: string }
  添加/删除模型
    → window.api.models.addModel(providerId, model) / removeModel(providerId, modelId)
    → ModelRegistry.addModel() / removeModel() → ConfigSync.sync()
```

---

## 14. ChatInputBox 重构

### 14.1 现有状态

ChatView 中的输入区域是简单的 textarea + Send 按钮，无工具栏。

### 14.2 新设计

参考 LobsterAI `CoworkPromptInput.tsx`（`主页.png` 底部），输入框 + 工具栏一体化设计。输入区在上，工具栏在下，整体为圆角卡片：

未选 Skills 时：
```
┌──────────────────────────────────────────────────────┐
│                                                      │
│  描述任务，/ 调用技能与工具，标准模式经济高效            │
│                                                      │
├──────────────────────────────────────────────────────┤
│ [📁 Downloads] [🛠] [🔌] [📎]         [⚡ 标准] [↑]  │
└──────────────────────────────────────────────────────┘
```

已选 Skills 时（输入框与工具栏之间插入 chip 标签条）：
```
┌──────────────────────────────────────────────────────┐
│                                                      │
│  描述任务，/ 调用技能与工具                             │
│                                                      │
│  [翻译助手 ×] [代码审查 ×] [+2]                       │
├──────────────────────────────────────────────────────┤
│ [📁 Downloads] [🛠³] [🔌] [📎]        [⚡ 标准] [↑]  │
└──────────────────────────────────────────────────────┘
```

**工具栏布局**：左右两端分布（`justify-between`）
- **左侧**（按使用频率排序）：工作目录选择 → Skills 选择 → 连接器（MCP）→ 附件添加
- **右侧**：模型选择器（下拉）、发送按钮
- 排序逻辑：环境上下文（cwd）→ 能力增强（skills）→ 外部集成（MCP）→ 输入增强（附件），认知负荷从左到右递增

**输入框行为**：
- 自动高度（`min-h-[24px]` → `max-h-[120px]`）
- 支持 `/` 斜杠命令触发 Skills 选择
- `Enter` 发送，`Shift+Enter` 换行
- placeholder 随当前模型模式变化：「描述任务，/ 调用技能与工具，标准模式经济高效」

### 14.3 子组件

| 组件 | 位置 | 功能 |
|------|------|------|
| `CwdSelector` | 工具栏左侧 | 工作目录选择器（Popover：最近目录列表 + `📁 添加文件夹` 按钮，选择系统目录） |
| `SkillSelector` | 工具栏左侧 | Skills 多选（Popover + checkbox），选中后工具栏图标显示 badge 数字，输入框下方显示 chip 标签条 |
| `ConnectorButton` | 工具栏左侧 | 连接器快捷入口（打开连接器弹窗，见 §12.13） |
| `AttachmentButton` | 工具栏左侧 | 附件添加按钮（拖放或点击选择文件，支持图片/文档） |
| `ModelSelector` | 工具栏右侧 | 模型选择下拉（⚡标准 / 🧠推理 + 具体模型列表） |
| `SendButton` | 工具栏右侧 | 发送按钮（圆形，`ArrowUp` 图标，`bg-accent`） |

### 14.4 CwdSelector 细节

参考 LobsterAI `主页3.png` 底部工具栏的「Downloads」工作目录展示：

```
┌──────────────────────┐
│  最近工作目录          │
│  📁 ~/Desktop         │
│  📁 ~/Downloads       │  ← 当前选中，高亮
│  📁 ~/projects/xxx    │
│  ─────────────────── │
│  📁 添加文件夹...      │  ← 打开系统目录选择器
└──────────────────────┘
```

- 工具栏展示：`[📁 目录名]`（只显示最后一级目录名，如 `Downloads`）
- 最近目录来源：`CoworkStore.getRecentWorkingDirs()`
- 新会话默认 cwd：用户 home 目录

### 14.5 SkillSelector 细节

**工具栏按钮状态**：

- 未选中：纯图标 `🛠`，`text-text-tertiary`
- 已选中：图标 + 右上角 badge 数字 `🛠³`，badge 为 `bg-accent text-white text-[10px] min-w-[16px] h-[16px] rounded-full`

**Popover 选择面板**（点击图标弹出）：

```
┌──────────────────────────┐
│  选择技能                 │
│                          │
│  ☑ 翻译助手              │
│  ☑ 代码审查              │
│  ☐ 文档生成              │
│  ☑ 数据分析              │
│  ☐ 图片理解              │
│                          │
│  已选 3 个                │
└──────────────────────────┘
```

- 列表来源：`window.api.skills.list()` 返回已启用的 skills
- 每行 = checkbox + skill 名称，点击切换选中
- 底部显示已选数量统计
- Popover 宽度 `w-[240px]`，最大高度 `max-h-[280px]` 超出滚动

**Chip 标签条**（输入框与工具栏之间）：

- 选中 ≥1 个 skill 时出现，`px-3 py-1.5` 内边距
- 每个 chip：`rounded-[8px] bg-bg-hover text-[12px] text-text-secondary px-2 py-0.5`，右侧 `×` 移除按钮
- 最多显示 3 个 chip，超出显示 `+N` 折叠标签（点击展开全部或打开 Popover）
- chip 移除时同步更新 Popover 中的 checkbox 状态
- 无选中时标签条隐藏，不占空间（`animate-collapse` 过渡 150ms）

### 14.6 模型选择器（ModelSelector）

工具栏右侧的模型选择器是快捷入口：

```
┌────────────────┐
│ ⚡ 标准（推荐） │ ← 默认，经济高效
│ 🧠 推理        │ ← 复杂任务
├────────────────┤
│ 切换模型 →      │ ← 展开具体模型列表
└────────────────┘
```

- 「标准」和「推理」是模式快捷切换，映射到 ModelRegistry 中配置的默认标准/推理模型
- 「切换模型」展开完整模型列表（按 Provider 分组），选择后覆盖当前会话模型
- 工具栏展示：`[⚡ 标准]` 或 `[🧠 推理]`，紧凑文字 + 图标
- ModelSelector 初始化时必须解析出一个当前模型：`directory.modelOverride` 优先，其次 `app_config.model.defaultProviderId/defaultModelId`
- ModelSelector 对外输出 `{ providerId, modelId }`，不直接输出 OpenClaw 字符串；OpenClaw provider id 映射只允许在主进程 `ModelRegistry` 内完成

### 14.7 数据流

```
用户输入消息 + 选择 cwd/skills/model
  → cowork:session:start({ prompt, cwd, skillIds, selectedModel, imageAttachments, pathReferences })
  → CoworkSessionManager 根据 cwd/useMainAgent 推导 agentId
  → DirectoryManager.ensureRegistered(cwd)（非 main agent 场景）
  → CoworkController 生成 sessionKey
  → ModelRegistry.toOpenClawModelRef(selectedModel)
  → Gateway sessions.patch({ key: sessionKey, model })
  → Gateway chat.send({ sessionKey, message, ... })
```

- `agentId` 是主进程根据目录策略派生出的运行时字段：main agent 使用私有 workspace，目录 agent 使用 `deriveAgentId(cwd)`，前端不提供独立的 Agent 选择控件
- `cwd` 来自 CwdSelector 选中的工作目录；如果用户未选择目录且默认目录为空，主进程回退到 main agent 私有 workspace
- `skillIds` 来自 SkillSelector 选中的技能列表
- `selectedModel` 来自 ModelSelector 选中的 `{ providerId, modelId }`；主进程转换成 OpenClaw model ref 后写入本地 session，并通过 `sessions.patch` 对 OpenClaw 当前 session 生效
- `selectedModel` 正常情况下不为空；如果前端无法解析可用模型，主进程回退到 `app_config.model` 并记录错误日志，不能把空模型传给 `sessions.patch`

---

## 15. 启动流程变更

### 15.1 index.ts 调整

Phase 2 在 Phase 1 的启动流程基础上插入 Manager 初始化：

```
Phase 1 步骤 1-4（DB、CoworkStore、EngineManager、ConfigSync）

+ Phase 2 新增:
  5. DirectoryManager 初始化
  6. ModelRegistry 初始化 + load()
  7. SkillManager 初始化 + scan()
  8. McpManager 初始化
  9. ImGatewayManager / CronJobService / MemorySearchConfigStore 初始化
  10. ConfigSync 聚合 Directory、Model、Skill、MCP、IM、Cron、memorySearch
  11. Manager/Service change 事件绑定 ConfigSync.sync()

Phase 1 步骤 5-15（窗口创建、BootCheck、IPC 注册等）

+ Phase 2 新增:
  注册新 IPC handlers（cowork、directory、models、skills、mcp、im）
```

---

## 16. 文件结构

### 16.1 新建文件

| 文件 | 职责 |
|------|------|
| `src/main/ai/directory-manager.ts` | 目录配置管理、目录 agentId 派生和 OpenClaw agents.list 输出 |
| `src/shared/models/provider-registry.ts` | 内置 Provider、OpenClaw provider id 映射和默认模型事实源 |
| `src/main/models/model-config-store.ts` | 模型 Provider、密钥和默认模型偏好持久化 |
| `src/main/models/model-registry.ts` | 模型 Provider 管理与 OpenClaw 配置转换 |
| `src/main/skills/skill-manager.ts` | Skill 扫描 + 开关控制 |
| `src/main/mcp/mcp-manager.ts` | MCP 服务器管理 |
| `src/main/memory/memory-search-config-store.ts` | 记忆搜索配置持久化 |
| `src/main/ipc/directory-ipc.ts` | 目录配置 IPC handlers |
| `src/main/ipc/models-ipc.ts` | Model IPC handlers |
| `src/main/ipc/skills-ipc.ts` | Skill IPC handlers |
| `src/main/ipc/mcp-ipc.ts` | MCP IPC handlers |
| `src/main/ipc/im-ipc.ts` | IM 实例、绑定和状态 IPC handlers |
| `src/renderer/src/chat/components/settings/SettingsPage.tsx` | Settings 全页面容器（左菜单 + 右内容） |
| `src/renderer/src/chat/components/settings/PreferenceSettings.tsx` | 偏好设置（语言/主题/字号/布局） |
| `src/renderer/src/chat/components/settings/ProfileSettings.tsx` | 个人资料设置 |
| `src/renderer/src/chat/components/settings/EngineSettings.tsx` | Agent 引擎设置 |
| `src/renderer/src/chat/components/settings/ModelSettings.tsx` | 模型配置 |
| `src/renderer/src/chat/components/settings/ImSettings.tsx` | 连接器设置（MCP/IM 完整管理） |
| `src/renderer/src/chat/components/settings/MemorySettings.tsx` | 记忆管理 |
| `src/renderer/src/chat/components/settings/DirectorySettings.tsx` | 工作目录配置 |
| `src/renderer/src/chat/components/settings/AboutSettings.tsx` | 关于页面 |
| `src/renderer/src/chat/components/settings/McpSettings.tsx` | MCP 服务详细管理 |
| `src/renderer/src/chat/components/ChatInputBox.tsx` | 增强输入框 |
| `src/renderer/src/chat/components/CwdSelector.tsx` | 工作目录选择器 |
| `src/renderer/src/chat/components/SkillSelector.tsx` | Skill 选择器 |
| `src/renderer/src/chat/components/McpPanel.tsx` | MCP 服务器管理面板 |
| `src/renderer/src/chat/components/ModelSelector.tsx` | 模型快捷选择器（输入框工具栏） |
| `src/renderer/src/chat/components/WelcomePage.tsx` | 欢迎页（空状态 + 功能引导卡片） |
| `src/renderer/src/chat/components/ConnectorPopup.tsx` | 连接器弹窗（MCP 快捷开关） |
| `src/renderer/src/chat/components/TaskMonitorPanel.tsx` | 右侧任务监控面板 |
| `src/renderer/src/chat/components/ChatHeader.tsx` | 聊天顶栏（会话标题 + 功能按钮） |
| `src/renderer/src/chat/components/SkillsPage.tsx` | 技能管理页面（Skill 列表 + 搜索 + 开关） |
| `src/renderer/src/chat/components/CronPage.tsx` | 定时任务页面（Phase 3 占位） |

### 16.2 修改文件

| 文件 | 变更 |
|------|------|
| `src/main/data/db.ts` | `initDatabase()` 新增/维护 directories、cowork_sessions、im_instances、im_conversation_bindings、im_session_mappings、scheduled_task_meta、mcp_servers |
| `src/main/ai/config-sync.ts` | 聚合 Directory、Model、Skill、MCP、IM、Cron、memorySearch 配置 |
| `src/main/ai/cowork-session-manager.ts` | 支持目录派生 agentId、main agent 回退、origin 和 sessionKey 格式 |
| `src/main/index.ts` | 启动流程新增 Manager 初始化 |
| `src/main/ipc/index.ts` | 注册新 IPC handlers |
| `src/preload/index.ts` | 新增/同步 cowork、directory、models、skills、mcp、im channels |
| `src/preload/index.d.ts` | 类型定义同步 |
| `src/renderer/src/chat/components/SettingsView.tsx` | 删除，被 settings/SettingsPage.tsx 替代 |
| `src/renderer/src/chat/components/Sidebar.tsx` | 重构：任务中心 + 频道分组 + 定时任务折叠 + settings 时隐藏 |
| `src/renderer/src/chat/ChatApp.tsx` | ViewType 升级（含 settings）, 新增状态模型, settings 时隐藏主侧栏 |
| `src/renderer/src/chat/components/ChatView.tsx` | 集成 ChatInputBox |

---

## 17. 验证标准

### 17.1 后端验证

- `npx tsc --noEmit` 类型检查通过
- DirectoryManager: ensureRegistered/list/update → DB 持久化 → ConfigSync 同步，目录 agentId 稳定派生
- ModelRegistry: 多 Provider 配置，API Key 不出现在 openclaw.json（只有 `${VAR}` 占位符）
- SkillManager: 扫描返回 Skill 列表，启用/禁用触发 ConfigSync
- McpManager: CRUD → DB 持久化 → ConfigSync 同步
- IM: 多实例平台（飞书/钉钉/企微）可创建多个实例，微信当前仅单实例；`im:create-instance` 必须按 `PLATFORM_INFO.maxInstances` 在后端拦截超额创建；对话绑定写入 `im_conversation_bindings`
- ConfigSync: 聚合 Directory、Model、Skill、MCP、IM、Cron、memorySearch → openclaw.json / AGENTS.md / exec-approvals.json 生成正确
- IPC channel 三处同步（ipc/*.ts + preload/index.ts + preload/index.d.ts）
- Manager change 事件 → ConfigSync.sync() 自动触发

### 17.2 前端验证

- **三栏布局**：侧栏（220px）+ 主内容区（flex-1）+ 右面板（240px 可收起）正确渲染
- **ViewType 路由**：`chat` / `skills` / `expert-kits` / `cron` / `im-channels` / `settings` 六个视图正确切换
- **侧栏导航联动**：点击「技能」→ SkillsPage，点击「专家套件」→ ExpertKitsPage，点击「定时任务」→ CronPage，点击「IM 频道」→ ImChannelsPage
- **任务中心侧栏**：左侧栏不展示工作目录分组；目录只在输入框、任务详情和右侧任务监控中作为上下文出现
- **`+ 新任务`**：点击 → 创建或聚焦草稿 → 用户输入未发送内容保存在草稿区 → 首次发送后创建 Cowork session 并移出草稿
- **任务│频道 Tab**：切换只改变侧栏列表内容，不改变主内容区路由
- **定时任务分组**：任务 Tab 中的定时任务区支持一级折叠；每个定时任务支持二级折叠并展示最近运行消息/session
- **任务列表**：只展示用户主动发起的普通 Cowork session，按 `updatedAt` 倒序展示；默认 Main 工作区不展示 metadata，非默认目录才展示目录短名
- **频道分组**：频道 Tab 对飞书/钉钉/企微按平台 → 实例 → 对话展示；微信当前单实例，按平台 → 对话展示
- **会话切换**：点击任务或频道下的 session → 主内容区切换到对应 Cowork 会话
- **欢迎页**：mascot + 标语 + 3 张功能引导卡片 + ChatInputBox
- **聊天顶栏**：会话标题（可编辑）+ 问题反馈链接 + 右面板 toggle
- **右面板（任务监控）**：toggle 展开/收起，显示产物/技能与MCP
- **连接器弹窗**：MCP 服务器快捷开关列表 + 底部跳转完整设置
- **技能页**：Skill 列表 + 搜索 + 开关切换
- **Settings 全页面**：「← 返回应用」+ 三段式分类菜单（通用/AI配置/扩展与集成）+ 卡片分组设置项
- **Settings 进入/返回**：齿轮 → 进入 settings（隐藏主侧栏）→ ← 返回应用（恢复主侧栏）
- **Settings 模型**：左侧 Provider 列表 + 右侧配置面板（API Key + Base URL + 测试连接）
- **Settings 工作目录**：目录列表 + 别名 + 模型覆盖 + 技能白名单
- **Settings 记忆**：MEMORY.md 查看/编辑/搜索
- **Settings MCP**：MCP 服务器详细管理（添加/编辑/删除 + 传输协议 + 配置）
- **ChatInputBox 工具栏**：左侧（cwd + skills + 连接器 + 附件）右侧（模型选择 + 发送）
- **ModelSelector**：标准/推理快捷切换 + 展开完整模型列表
- **CwdSelector**：最近目录列表 + 添加文件夹
- **消息数据流**：`cowork.onMessage` → 消息追加，`cowork.onMessageUpdate` → 流式更新，`cowork.onComplete` → 完成态
- 所有 UI 组件遵循 PetClaw 设计系统（Tailwind token、圆角、动效）
