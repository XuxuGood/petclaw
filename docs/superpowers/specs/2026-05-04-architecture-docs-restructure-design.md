...
**日期**: 2026-05-04  
**状态**: 待实施  
**范围**: `docs/架构设计/`、`docs/设计/`、架构文档命名、模块文档模板
...
同时，`docs/设计/` 目前主要保存外部产品参考图。这些素材可以辅助判断，但不应被误认为
PetClaw 当前实现或目标视觉规范。
...
5. 明确 `docs/设计/` 中外部参考、PetClaw 视觉规范和当前实现截图的边界。
...
`petclaw-desktop` 的详细架构文档，是当前主体。按功能模块拆分，并保留少量 desktop
专属工程边界文档。
...
`Desktop架构设计.md` 是 desktop 领域地图，讲 Main、Preload、Renderer、Pet Window、
SQLite、OpenClaw runtime、系统集成之间的关系。它不替代各功能模块文档。

`Renderer架构设计.md` 是 desktop 前端横切规范，讲页面边界、状态模型、全局布局、
...
1. `desktop/Renderer架构设计.md` 描述 desktop renderer 的全局架构、布局体系、状态模型和交互规则。
...
PetClaw 自己的视觉、组件和页面布局规范，是 UI 实现依据。
...
1. 先读 `docs/架构设计/desktop/Renderer架构设计.md`，理解状态和交互边界。
2. 再读 `docs/设计/specs/` 对应视觉规范。
3. 必要时查看 `docs/设计/references/` 获取风格参考，但不得直接复制外部产品。
4. 修改后更新关键页面截图到 `docs/设计/snapshots/`，作为视觉回归依据。
...
`desktop/Desktop打包架构设计.md` 只讲 Electron Builder、本地 runtime 打包、notarize
...
→ 拆分为：
  ...
  - docs/架构设计/desktop/Desktop架构设计.md
...
风险：AI Agent 把 `docs/设计/references/` 的外部产品图当成 PetClaw 规范。

应对：`docs/设计/README.md` 必须明确 `references/` 只作参考；实现依据以
`specs/` 和 `snapshots/` 为准。
...
5. `docs/设计/README.md` 明确 `references/`、`specs/`、`snapshots/` 的事实源边界。
