...
- FIFO 队列是全局串行展示，但每个请求必须保留 `sessionId`、`requestId`、`toolUseId` 和 tool 上下文，用于来源标注、会话停止/删除清理和避免审批残留。
...
Renderer 消费 `cowork:stream:*` 时必须以 `sessionId` 过滤当前会话详情；后台会话事件只更新运行状态、未读或列表摘要。`messageUpdate` 必须优先按 `messageId` 合并，缺失 `messageId` 时才允许退化为当前会话最后一条 assistant 消息。会话详情打开时使用 `cowork:session:get` 获取 snapshot，并与后续 stream 合并，旧 snapshot 不能覆盖已经切换的新会话。
...
### 15.7 前端 UI（参考设计稿 `docs/设计参考/references/定时任务/`）
...
| 阶段 | 时机 | 注册函数 | 依赖 |
|------|------|----------|------|
| **Phase A** | db 就绪后、runBootCheck 前 | `registerBootIpcHandlers`（i18n:*、app:version）+ `registerSettingsIpcHandlers`（settings:*）+ index.ts 闭包（boot:status、boot:retry） | 仅 db / managers |
...
**技能子菜单**（参考设计稿 `docs/设计参考/references/对话框功能/`）：
...
│     弹窗在所有视图分支（bootcheck/settings/main）之上渲染                │
└────────────────────────────┬─────────────────────────────────────────────┘
                             │ 用户点击选项
                             ▼
┌──────────────────────────────────────────────────────────────────────────┐
...
app.whenReady()
  ...
  → boot:complete → ChatApp main → app:pet-ready
  → createPetWindow()
...
│   │   │   ├── boot-ipc.ts            #   启动阶段 IPC（i18n、app:version）
│   │   │   ├── chat-ipc.ts
│   │   │   ├── directory-ipc.ts
│   │   │   ├── skills-ipc.ts
│   │   │   ├── models-ipc.ts
...
**改造范围**：
- Settings / Chat / Sidebar / Tray 的用户可见文案改为 `t(key, params?)`
...
- Settings / Chat / Sidebar / Tray 用户可见文案无硬编码中英文
