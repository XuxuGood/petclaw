...
BootCheck 是 Main Window 的启动阶段界面。BootCheck 阶段只允许调用 Phase A IPC：`boot:*`、`i18n:*`、`settings:*`、`app:version`、`logging:*`。完整业务 IPC 必须在 boot 成功、runtime services 就绪后、`boot:complete` 通知 renderer 前注册。Renderer 进入主界面后再发送 `app:pet-ready`，该事件用于创建 Pet Window、双窗口桥接和快捷键，不负责注册业务 IPC，也不得重新激活 Main Window 或抢夺焦点。Boot 失败时 UI 必须停留在可恢复状态，禁止展示可发送 Cowork 的入口；失败恢复区可以提供重试、打开日志目录和导出诊断包等不依赖 runtimeServices 的动作。
