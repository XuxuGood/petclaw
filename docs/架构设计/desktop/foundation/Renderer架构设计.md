...
Pet Window 只消费主进程 `PetEventBridge` 聚合后的 `pet:state-event` 和 `pet:bubble`。它不能理解 Chat、IM、Cron、HookServer、CoworkController 的内部状态，也不能直接请求业务数据。点击宠物只允许触发窗口级行为：切换 Main Window；若宠物处于 Sleep 状态，同一次点击可额外唤醒宠物。Pet Window 不抢输入焦点。

Chat 页面负责把 `ChatInputBox` 的真实 DOM bounds 通过 `window:composer-bounds:update` 上报给主进程，
用于 Pet Window 首次锚到聊天输入区右上方附近。该 IPC 只传布局坐标 `{ x, y, width, height }`，不传
prompt、附件、cwd、模型或会话内容；用户拖拽 Pet 后，主进程恢复保存的 Pet 位置优先于 renderer 上报的
composer bounds。Pet Window 的透明窗口尺寸大于猫体可见像素；主进程定位时必须按猫 sprite 的可见边界
计算视觉锚点，不能直接用透明 BrowserWindow 外框贴齐输入框。

Cat 动画素材使用 webm 源文件 + png sprite atlas 的构建管线。源素材放在
`petclaw-desktop/src/renderer/src/assets/cat/*.webm`，运行
`pnpm --filter petclaw-desktop assets:cat-sprites` 后生成：

- `petclaw-desktop/src/renderer/src/assets/cat-sprites/*.png`
- `petclaw-desktop/src/renderer/src/pet/cat-sprite-manifest.ts`

新增或替换猫咪 webm 时，必须同步 `petclaw-desktop/scripts/build-cat-sprites.cjs`
里的 `ANIMATIONS` 列表，重新运行 `assets:cat-sprites`，并提交生成后的 sprite png
和 manifest。睡眠唤醒素材 `sleep-leave` 曾出现孤立竖线伪影；构建脚本包含该素材的
细线清理逻辑，`petclaw-desktop/tests/renderer/pet/cat-sprites.test.ts` 是对应回归测试，
修改睡眠相关 webm 后必须运行该测试。

### 2.3 BootCheck

BootCheck 是 Main Window 的启动阶段界面。BootCheck 阶段只允许调用 Phase A IPC：`boot:*`、`i18n:*`、`settings:*`、`app:version`、`logging:*`。完整业务 IPC 必须在 boot 成功、runtime services 就绪后、`boot:complete` 通知 renderer 前注册。Renderer 进入主界面后再发送 `app:pet-ready`，该事件用于创建 Pet Window、双窗口桥接，并在 Pet Window ready 后恢复 Main Window 前台激活，不负责注册业务 IPC。Boot 失败时 UI 必须停留在可恢复状态，禁止展示可发送 Cowork 的入口；失败恢复区可以提供重试、打开日志目录和导出诊断包等不依赖 runtimeServices 的动作。
...
- `window:composer-bounds:update` 仅允许 renderer 上报聊天输入框相对 Main Window 内容区的矩形坐标，
  主进程据此定位 Pet Window；该 channel 不承载业务数据，也不允许 renderer 直接设置任意窗口位置。
