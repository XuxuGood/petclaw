...
BootCheck 是 Main Window 的启动阶段界面。BootCheck 阶段只允许调用 Phase A IPC：`boot:*`、`i18n:*`、`settings:*`、`app:version`。完整业务 IPC 必须在 boot 成功、runtime services 就绪后、`boot:complete` 通知 renderer 前注册。Renderer 进入主界面后再发送 `app:pet-ready`，该事件用于创建 Pet Window、双窗口桥接，并在 Pet Window ready 后恢复 Main Window 前台激活，不负责注册业务 IPC。Boot 失败时 UI 必须停留在可恢复状态，禁止展示可发送 Cowork 的入口。
