...
外部参考图可放在本地 `docs/设计参考/references/`，只作风格参考；当前实现截图可放在本地 `docs/设计参考/snapshots/`，作为本机视觉回归基线。`docs/设计参考/` 不提交到仓库。
...
4. 本地 `docs/设计参考/snapshots/` 当前截图。
5. 本地 `docs/设计参考/references/` 外部参考图。
...
- UI 改动影响关键页面外观时，可更新本地 `docs/设计参考/snapshots/desktop/` 辅助回归；该目录不提交。
