...
│  Dock menu          -> Open / Show-Hide Pet / Settings              │
│  Application menu   -> About / Settings / Open / Show-Hide / Quit   │
│  Pet context menu   -> Pause-Resume / Quit                          │
...
- Pet Window 是 non-focusable floating overlay：可以 hover、点击、右键和拖拽，但不抢 Main Window 的输入焦点，也不出现在 Dock/Window 菜单的窗口切换列表里。
- Main Window 是主工作台：首次创建必须按工作区居中，用户移动或缩放后恢复保存的
  Main bounds。Pet Window 是辅助浮层，不能反向决定 Main Window 的位置。
- Pet Window 首次创建且 Main Window 可见时，应优先使用 renderer 通过
  `window:composer-bounds:update` 上报的真实 `ChatInputBox` bounds，锚到聊天输入区右上方附近，并播放
  renderer 初始 `begin -> static` 出场动画。锚点按猫 sprite 在 180x145 透明 Pet Window 内的可见像素边界
  计算，避免透明留白导致视觉上偏离输入框。若 composer bounds 尚未上报，主进程只允许使用 Main Window
  bounds 做临时估算；程序自动锚点不得写入 `window.petPosition`。用户拖拽 Pet 后只恢复 Pet 自己的保存
  位置；点击宠物显示或隐藏 Main Window 时不得自动移动 Main Window 或 Pet Window。

## 6. 数据模型

系统集成保存窗口布局、用户偏好和更新状态。窗口布局偏好统一写入 SQLite
`app_config`，不再使用独立 JSON settings 文件。平台特有差异必须封装在 main process 内部。

推荐数据边界：

| 数据 | 所有者 | 持久化 | 说明 |
|---|---|---|---|
| main window bounds | main process | SQLite `app_config.window.mainBounds` | 仅保存可恢复尺寸和位置 |
| pet window bounds | main process | SQLite `app_config.window.petPosition` | 与 Pet 事件状态分离；只恢复 Pet 自身位置，不影响 Main Window bounds |
| composer bounds | renderer -> main process | memory | ChatInputBox 的布局坐标，只用于首次 Pet 锚点计算；结合猫 sprite 可见边界换算到屏幕坐标，不持久化、不包含业务内容 |
...
| 资产 | 用途 | 约束 |
|---|---|---|
| `resources/icon.png` | 设计源 | 1024px 高分辨率 PNG；只在图标设计变化时重新生成平台产物 |
| `build/icons/mac/icon.icns` | macOS Dock/Finder/Launchpad | `electron-builder.json#mac.icon` 输入，覆盖 16、32、64、128、256、512、1024 尺寸 |
| `build/icons/win/icon.ico` | Windows exe/taskbar/shortcut | `electron-builder.json#win.icon` 输入，包含 16、24、32、48、64、128、256 尺寸 |
| `build/icons/png/` | Linux desktop/AppImage/deb | `electron-builder.json#linux.icon` 输入，包含多尺寸 PNG |
| About/Settings 小图标 | 应用内部品牌点 | 可使用简化彩色版，不做导航装饰 |

图标方向使用“抽象爪痕 + AI 光点”：表达 PetClaw 和 AI 协作，避免真实猫脸、`PC` 文字、机器人头或复杂代码符号。
平台图标产物由 `pnpm --filter petclaw-desktop assets:icons` 从源图生成并提交；`package:dir` 和
`dist:*` 只运行 `check:icons`，不在每次构建时重生成。
...
Dock 右键菜单会由 macOS 自动补充应用身份、窗口列表和 Quit 等系统项；PetClaw 的自定义
Dock Menu 只添加不重复应用名的核心动作。Pet Window 是透明桌面覆盖层，必须从 macOS
shown windows menu 排除，避免 Dock 右键窗口列表出现两个同名 PetClaw。
...
Application Menu / Edit
  ├── Undo / Redo
  ├── Cut / Copy / Paste / Paste and Match Style / Delete
  └── Select All
...
- 主窗口必须注册 `CmdOrCtrl` 编辑快捷键兜底，确保 macOS `Command+C` 和 Windows/Linux
  `Ctrl+C` 都能复制 renderer 中已选中的普通文本；输入框的剪切、粘贴、全选、撤销和重做也应保留。
...
- 所有用户可见菜单项必须走 i18n。
- 推荐使用 `system.*` key 表达系统动作，例如 `system.open`、`system.openPetClaw`、`system.showPet`、`system.hidePet`、`system.pausePet`、`system.resumePet`、`system.settings`、`system.quit`、`system.about`。
...
Dock Menu、Application Menu 和非 macOS fallback tray 必须在主窗口创建后、BootCheck
...
和 Dock/Finder 图标以 packaged `.app` 为事实源，快速本地系统壳验收使用
`pnpm --filter petclaw-desktop run package:dir` 生成当前平台 unpacked app。主进程只在 `initializeMacosIntegration`
内显式设置 `app.setName('PetClaw')`、`app.setActivationPolicy('regular')`、
`app.dock.show()`；`app.dock.setIcon(...)` 只用于 dev runner 识别开发态 Dock 图标，
packaged app 不在运行时覆盖 Dock 图标，确保 `.app` bundle 的 `.icns` 是正式包唯一事实源。
正式包不手写 `CFBundleIconName`；macOS plist 图标入口由 electron-builder 根据
`mac.icon` 自动生成 `CFBundleIconFile=icon.icns`，afterPack hook 只校验资源和刷新 bundle
元数据。
`pnpm --filter petclaw-desktop dev:openclaw` 启动时仍需确保 Dock Menu 和 Application Menu
模板可用。
...
| `src/main/system/edit-shortcuts.ts` | 主窗口 `CmdOrCtrl` 编辑快捷键兜底 |
...
| `electron-builder.json` | 平台图标、协议和签名相关资源路径 |
| `resources/` | 源图标、tray 和其它运行时资源 |
| `build/icons/` | macOS/Windows/Linux 平台化图标产物 |
...
- Application Menu 必须保留标准 Edit 角色，主窗口必须覆盖 `CmdOrCtrl` 编辑快捷键。
