...
`ai:doctor` 是深度诊断入口；`ai:tools:check` 是快速状态检查入口。两者都不安装、不写配置。
...
```bash
pnpm ai:doctor
pnpm --silent ai:doctor -- --json
pnpm ai:doctor -- --deep
```

职责：

- 检查 Node.js、Git、pnpm、uv。
- 检查项目级 `.mcp.json` 和 MCP 模板目录。
- 检查 GitNexus registry 是否存在、可读、可写。
- 检查 `.gitnexus/lbug` 是否被其它进程占用，只报告 PID 和命令，不自动结束进程。
- 检查 GitNexus 命令入口、registry/list、当前仓库 alias、status 和 context smoke test。
- 检查 Serena 命令入口和当前 dashboard 地址。
- `--deep` 时额外运行 `serena project health-check`，确认 TypeScript LSP、symbol 和引用能力可用。
- `--json` 时输出机器可读诊断结果，供 AI 客户端和 CI 解析。

该命令是 AI Context Harness 的统一诊断入口。GitNexus 锁、registry 权限、Serena dashboard 未监听等问题都归类为工具链状态，不归类为业务代码风险。

### 6.4 `pnpm ai:tools:check`
...
它只做快速检查；需要锁、权限、MCP smoke test 或 Serena health-check 时使用 `pnpm ai:doctor`。
...
| `scripts/ai/check-ai-tools.cjs` | 工具链只读快速检查 |
| `scripts/ai/doctor-ai-context.cjs` | AI Context Harness 深度诊断 |
