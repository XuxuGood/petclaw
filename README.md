...
pnpm --filter petclaw-desktop dev:openclaw          # 首次运行：自动 clone openclaw 源码 + 完整构建
                                                    # 内部执行顺序：
                                                    #   1. ensure-openclaw-version   — git clone ../openclaw && checkout 锁定版本
                                                    #   2. build-openclaw-runtime    — pnpm install → tsc → 打包 asar → 生成入口
                                                    #   3. sync-openclaw-current     — symlink current/ → 当前平台目录
                                                    #   4. bundle-openclaw-gateway   — esbuild 打包 gateway
...
pnpm --filter petclaw-desktop dist:linux            # → release/PetClaw-x.y.z.AppImage
...
| `AGENTS.md` | Codex 工作指南 |
| `docs/架构设计/PetClaw总体架构设计.md` | 总体架构事实源（六层架构、数据模型、模块设计） |
