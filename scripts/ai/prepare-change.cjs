...
function ensureFreshEnoughIndex() {
  ...
  if (!isGitNexusRepoRegistered()) {
    ...
    logInfo(`GitNexus 仓库索引未注册到当前别名，准备注册：${getGitNexusRepoName()}`)
  ...
  logInfo('GitNexus 索引可能已过期，准备变更前先刷新。')
  const analyze = runGitNexusForRepo(['analyze'])
  if (isGitNexusEnvironmentError(analyze)) {
    logWarn(describeGitNexusEnvironmentError(analyze))
    return {
...
function runTextFallback(target) {
  ...
  logWarn('GitNexus 未命中可用 symbol，改用前后端文本使用面扫描。')
...
function main() {
  ...
  logInfo(`准备分析改动目标：${target}`)
  const indexState = ensureFreshEnoughIndex()
  if (!indexState.available) {
    logWarn('无法使用最新 GitNexus 索引，下面输出人工核对清单。')
  }

  if (indexState.environmentBlocked) {
    logWarn('GitNexus 锁/权限问题属于工具链环境异常，不视为业务风险；已自动降级为本地使用面扫描。')
    runTextFallback(target)
  }
  ...
  if (!indexState.environmentBlocked) {
    ...
    if (isGitNexusEnvironmentError(context) || isGitNexusEnvironmentError(impact)) {
      logWarn(describeGitNexusEnvironmentError(isGitNexusEnvironmentError(context) ? context : impact))
      logWarn('GitNexus 锁/权限问题属于工具链环境异常，不视为业务风险；已自动降级为本地使用面扫描。')
      runTextFallback(target)
  ...
  if (hasWorkingTreeChanges()) {
    logInfo('工作区已有未提交变更，AI 修改前需要确认这些变更是否属于当前任务。')
  }
